...
import { Lightbulb, CheckCircle, AlertCircle, Edit, Check, X, Loader2, Sparkles } from 'lucide-react';
...
export default function CoreOfferOutlineSectionEditor({ 
  ...
  const [sectionEvaluations, setSectionEvaluations] = useState<Record<number, SectionEvaluation>>({});
  const [sectionRewrites, setSectionRewrites] = useState<Record<number, SectionRewrite>>({});
  const [evaluatingSection, setEvaluatingSection] = useState<number | null>(null);
  const [rewritingSection, setRewritingSection] = useState<number | null>(null);
  const [showingRewriteFor, setShowingRewriteFor] = useState<number | null>(null);
  ...
  const shouldShowAIFeatures = (sectionTitle: string): boolean => {
    ...
    const normalizedTitle = sectionTitle
      ...
      .replace(/[\u{1F300}-\u{1F9FF}]|[\u{2600}-\u{26FF}]|[\u{2700}-\u{27BF}]|[\uFE00-\uFE0F]|[\u{1F1E6}-\u{1F1FF}]/gu, '') // Remove other emojis
      .replace(/\s+/g, ' ')
      .trim()
      .toUpperCase();
  ...
  const evaluateSectionMutation = useMutation({
    mutationFn: async ({ sectionIndex, sectionTitle, sectionContent }: { sectionIndex: number; sectionTitle: string; sectionContent: string }) => {
      const endpoint = offerType === 'tripwire' 
        ? '/api/tripwire-outline/evaluate-section'
        : '/api/core-offer-outline/evaluate-section';
      
      const bodyKey = offerType === 'tripwire' ? 'mainOffer' : 'mainTransformation';
  ...
  const rewriteSectionMutation = useMutation({
    mutationFn: async ({ sectionIndex, sectionTitle, sectionContent, specificIssues }: { sectionIndex: number; sectionTitle: string; sectionContent: string; specificIssues: string[] }) => {
      const endpoint = offerType === 'tripwire'
    ...
    onSuccess: (data, variables) => {
      ...
      if (cleanedContent && section) {
        ...
        const headingPatterns = [
          ...
          new RegExp(`^##?\\s*${section.title.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s*[\\r\\n]+`, 'im'),
          ...
          new RegExp(`^${titleText.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s*[\\r\\n]+`, 'im'),
          // "Section Title:" prefix variations
          new RegExp(`^Section Title:\\s*${section.title.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s*[\\r\\n]+`, 'im'),
          new RegExp(`^Section Title:\\s*${titleText.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s*[\\r\\n]+`, 'im'),
          // Just the emoji and number at the start
          new RegExp(`^\\d+[️⃣]?\\s*${titleText.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s*[\\r\\n]+`, 'im'),
  ...
  const evaluateSection = (sectionIndex: number, sectionsToEvaluate: Section[] = sections) => {
  ...
  const renderTextWithBold = (text: string) => {
    ...
    return text.split('\n').map((line, lineIdx) => {
      ...
      if (line.startsWith('**') && line.endsWith('**')) {
        return <p key={lineIdx} className="font-bold my-0.5">{line.slice(2, -2)}</p>;
      } 
      ...
      if (line.trim().startsWith('-') || line.trim().startsWith('•')) {
        return <p key={lineIdx} className="ml-4 my-0.5">{line}</p>;
      }
      ...
      // Regular text
      return <p key={lineIdx} className="my-0.5">{line}</p>;
    });
  };
  ...
  return (
    <div className="space-y-3">
      {sections.map((section, index) => {
        ...
        if (section.title === 'CORE OFFER OUTLINE' || (section.level === 1 && section.content.trim().length < 10)) {
          return null;
        }
        ...
        return (
          <Card key={index} className="p-4 border">
            ...
            {showingRewriteFor !== index && (
              <div className="flex items-start justify-between mb-3">
                <div className="flex-1">
                  <h3 className={`font-bold ${section.level === 1 ? 'text-xl' : section.level === 2 ? 'text-lg' : 'text-base'} text-slate-900 mb-1`}>
                    {section.title}
                  </h3>
                  
                  {/* Quality Score Badge - Only show for AI-enabled sections */}
                  {evaluation && !isEditing && shouldShowAIFeatures(section.title) && (
                    <div className="flex items-center gap-2 flex-wrap">
                      <div className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full border text-xs ${getScoreBgColor(evaluation.totalScore / 4)}`}>
                        <span className="font-semibold">Quality: {evaluation.totalScore}/20</span>
                      </div>
                      {evaluation.needsRewrite && (
                        <div className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-orange-100 border border-orange-300">
                          ...
                          <span className="text-xs font-medium text-orange-700">Needs Improvement</span>
                        </div>
                      )}
                    </div>
                  )}
            ...
            {evaluation && !isEditing && shouldShowAIFeatures(section.title) && showingRewriteFor !== index && (
              <div className="space-y-2.5 mt-2 pt-2 border-t">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                  {Object.entries(evaluation.categoryScores).map(([category, data]) => (
                    <div key={category} className={`p-2 rounded border ${getScoreBgColor(data.score)}`}>
                      <div className="text-xs font-medium text-slate-600 capitalize">{category}</div>
                      <div className={`text-base font-bold ${getScoreColor(data.score)}`}>{data.score}/5</div>
                    </div>
                  ))}
                </div>
                  ...
                  <div className="bg-green-50 p-2.5 rounded border border-green-200">
                    ...
                    <ul className="space-y-0.5">
                      {evaluation.strongPoints.map((point, idx) => (
                        <li key={idx} className="text-xs text-green-800">• {point}</li>
                      ))}
                    </ul>
                  </div>
                )}
                  ...
                  <div className="bg-orange-50 p-2.5 rounded border border-orange-200">
                    ...
                    <ul className="space-y-0.5">
                      {evaluation.needsWork.map((point, idx) => (
                        <li key={idx} className="text-xs text-orange-800">• {point}</li>
                      ))}
                    </ul>
                  </div>
                )}
                ...
                {/* Alignment Issues */}
                {evaluation.alignmentIssues && evaluation.alignmentIssues.length > 0 && (
                ...
                {evaluation.coachingFeedback && (
                  <div className="bg-blue-50 p-2.5 rounded border border-blue-200">
                    ...
                    <p className="text-xs text-blue-800 whitespace-pre-wrap leading-relaxed">{evaluation.coachingFeedback}</p>
                  </div>
                )}

                {/* Rewrite Section - Always show for AI-enabled sections */}
                {shouldShowAIFeatures(section.title) && !rewrite && section.title !== 'CORE OFFER OUTLINE' && section.level !== 1 && (
                  <Button
                    size="sm"
                    onClick={(e) => {
                      e.preventDefault();
                ...
                {rewrite && section.title !== 'CORE OFFER OUTLINE' && section.level !== 1 && (
                  <div className="space-y-2">
                    ...
                    <div className="bg-purple-50 p-2.5 rounded border border-purple-300 space-y-2">
                      ...
                      <div className="text-xs text-purple-800">
                        <p className="font-medium mb-0.5">Why this works better:</p>
                        <p className="italic text-xs">{rewrite.rationale}</p>
                      </div>
                      
                      {rewrite.improvements?.length > 0 && (
                        <div className="text-xs">
                          <p className="font-medium text-purple-900 mb-0.5">Key Improvements:</p>
                          <ul className="space-y-0.5">
                            {rewrite.improvements.map((imp, idx) => (
                              <li key={idx} className="text-purple-800 text-xs">✓ {imp}</li>
                            ))}
                          </ul>
                        </div>
                      )}
