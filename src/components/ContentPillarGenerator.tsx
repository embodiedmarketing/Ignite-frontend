...
const contentPreferencesSchema = z.object({
  ...
  desiredFeelings: z
    .string()
    .min(10, "Describe how you want people to feel (at least 10 characters)"),
  avoidFeelings: z
    .string()
    .min(5, "Describe what you don't want people to feel"),
  brandAdjectives: z.string().min(10, "List 3-5 brand adjectives"),
  coreThemes: z
    .string()
    .min(20, "Describe 3-4 core themes your audience needs to understand"),
  problemsMyths: z
    .string()
    .min(20, "Describe common problems, mistakes, or myths to address"),
  valuesBeliefs: z
    .string()
    .min(15, "Describe values or beliefs to consistently reinforce"),
  contrarianTakes: z
    .string()
    .min(20, "Describe 3-5 contrarian or disruptive takes"),
  actionableTips: z
    .string()
    .min(15, "Provide quick tips, tools, or frameworks"),
  commonObjections: z
    .string()
    .min(15, "Describe common questions or objections"),
  beliefShifts: z
    .string()
    .min(20, "Describe belief shifts your audience needs to make"),
  authenticTruths: z
    .string()
    .min(20, "Share a truth you want to shout from the rooftops"),
  keyMessage: z
    .string()
    .min(15, "What's the one thing you want your audience to remember?"),
...
export default function ContentPillarGenerator() {
  ...
  const [generatedStrategy, setGeneratedStrategy] =
    useState<GeneratedContentStrategy | null>(null);
  ...
  const { data: messagingStrategy, refetch: refetchMessagingStrategy } =
    useQuery<MessagingStrategy | null>({
  ...
  useEffect(() => {
    ...
    const mapInsightsToForm = (insights: any): Partial<ContentPreferences> => {
      ...
      if (insights.desiredFeelings)
        mapped.desiredFeelings = String(insights.desiredFeelings);
      if (insights.avoidFeelings)
        mapped.avoidFeelings = String(insights.avoidFeelings);
      if (insights.brandAdjectives)
        mapped.brandAdjectives = String(insights.brandAdjectives);
      if (insights.coreThemes) mapped.coreThemes = String(insights.coreThemes);
      if (insights.problemsMyths)
        mapped.problemsMyths = String(insights.problemsMyths);
      if (insights.valuesBeliefs)
        mapped.valuesBeliefs = String(insights.valuesBeliefs);
      if (insights.contrarianTakes)
        mapped.contrarianTakes = String(insights.contrarianTakes);
      if (insights.actionableTips)
        mapped.actionableTips = String(insights.actionableTips);
      if (insights.commonObjections)
        mapped.commonObjections = String(insights.commonObjections);
      if (insights.beliefShifts)
        mapped.beliefShifts = String(insights.beliefShifts);
      if (insights.authenticTruths)
        mapped.authenticTruths = String(insights.authenticTruths);
      if (insights.keyMessage) mapped.keyMessage = String(insights.keyMessage);
      if (insights.authenticVoice)
        mapped.authenticVoice = String(insights.authenticVoice);
      ...
      try {
        ...
        if (parsed.extractedInsights) {
          ...
          const mergedData: ContentPreferences = {
            ...formData,
            ...Object.fromEntries(
              Object.entries(insightsMapping).filter(
                ([key, value]) =>
                  value &&
                  (!formData[key as keyof ContentPreferences] ||
                    formData[key as keyof ContentPreferences] === "")
              )
            ),
          ...
          console.log(
            "✅ Populated Content Strategy form with latest transcript insights"
          );

          toast({
            title: "Form updated",
            description:
              "Latest interview insights have been loaded into the form fields.",
          });
  ...
  useEffect(() => {
      ...
      if (insightsData?.extractedInsights) {
        ...
        if (Object.keys(updates).length > 0) {
          ...
          toast({
            title: "Form updated",
            description:
              "Latest interview insights have been loaded into empty form fields.",
          });
        }
      }
    };
    ...
    window.addEventListener(
      "transcriptInsightsUpdated",
      handleTranscriptInsightsUpdate as EventListener
    );
  ...
  useEffect(() => {
    ...
    try {
      ...
      const strategyData =
        typeof savedGeneratedStrategy.contentPillars === "string"
          ? JSON.parse(savedGeneratedStrategy.contentPillars)
          : savedGeneratedStrategy.contentPillars;

      const ideasData =
        typeof savedGeneratedStrategy.contentIdeas === "string"
      ...
      console.log(
        "[ContentPillarGenerator] Loaded saved strategy and ideas from database"
      );
    } catch (error) {
  ...
  usePageLeaveWarning(generateStrategyMutation.isPending || isGeneratingIdeas);
  ...
  const generateContentIdeasAfterStrategy = async (
    strategy: GeneratedContentStrategy
  ) => {
    ...
    try {
      ...
      if (userId && data.ideas && data.ideas.length > 0 && strategy) {
        try {
          ...
          console.log(
            "[ContentPillarGenerator] Saved strategy and ideas to database"
          );
        } catch (error) {
        ...
        console.log(
          "[ContentPillarGenerator] Skipping save - conditions not met:",
          {
            hasUserId: !!userId,
  ...
  const handleGenerateContentIdeas = async () => {
    ...
    if (!latestMessagingStrategy) {
      toast({
        ...
        description:
          "Please complete your messaging strategy first before generating content ideas.",
        variant: "destructive",
      });
      return;
    }
  ...
  const convertStrategyToMarkdown = (
    strategy: GeneratedContentStrategy
  ): string => {
    ...
    if (
      strategy.voiceAuthenticityGuide &&
      strategy.voiceAuthenticityGuide.length > 0
    ) {
  ...
  const convertStrategyToText = (): string => {
    ...
    if (
      generatedStrategy.contentGoals &&
      generatedStrategy.contentGoals.length > 0
    ) {
    ...
    if (
      ...
      generatedStrategy.contentPillars.length > 0
    ) {
    ...
    if (
      generatedStrategy.disruptiveAngles &&
      generatedStrategy.disruptiveAngles.length > 0
    ) {
    ...
    if (
      generatedStrategy.voiceAuthenticityGuide &&
      generatedStrategy.voiceAuthenticityGuide.length > 0
    ) {
    ...
    try {
      ...
      if (
        ...
        generatedStrategy.contentPillars.forEach((pillar, index) => {
          ...
          if (pillar.connectionToOffer) {
            addText(
              `Connection to Offer: ${pillar.connectionToOffer}`,
              9,
              "normal"
            );
          }
  ...
  const handleDownloadContentIdeasPDF = async () => {
    ...
    try {
      ...
      contentIdeas.forEach((idea, index) => {
        ...
        addText(
          `Emotional Intention: ${idea.emotionalIntention}`,
          10,
          "normal"
        );
  ...
  const handleCopyCombinedToClipboard = async () => {
    if (!generatedStrategy || !contentIdeas || contentIdeas.length === 0)
      return;
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-600">
              Your personalized, AI-generated content strategy
            </p>
          </div>
        ...
        <Card className="border border-slate-200">
          <CardContent className="p-12">
            ...
            <div className="mb-8 flex items-start justify-between">
              <div>
                <h2 className="text-3xl font-bold text-coral-700 mb-6">
            ...
            <h2 className="text-2xl font-bold text-coral-700 mb-6 pb-4 border-b-2 border-slate-300">
              PART 1: CONTENT STRATEGY PLAN
            </h2>
                  ...
                  {isEditMode && editedStrategy ? (
                    ...
                    <ol className="space-y-3 pl-6">
                      {generatedStrategy.contentGoals.map((goal, index) => (
                        <li
                          key={index}
                          className="text-lg text-slate-800 list-decimal"
                  ...
                  {generatedStrategy.brandVibe
                    ...
                    .map((line, index) => (
                      <p
                        key={index}
                        className={
                          line.includes("Tone statement:") ||
                          line.includes("helping")
                            ? "italic"
                            : ""
                        }
                        dangerouslySetInnerHTML={{
                  ...
                  <div className="space-y-4">
                    {generatedStrategy.contentPillars.map((pillar, index) => (
                      <div key={index} data-testid={`card-pillar-${index}`}>
                        ...
                        <p
                          className="text-base text-slate-700 leading-relaxed mb-1"
                          dangerouslySetInnerHTML={{
              ...
              generatedStrategy.voiceAuthenticityGuide.length > 0 && (
                <div>
                  ...
                  <ul className="space-y-3 pl-6">
                    {generatedStrategy.voiceAuthenticityGuide.map(
                      (bullet, index) => (
                        <li
                          key={index}
                          className="text-lg text-slate-800 list-disc"
                          dangerouslySetInnerHTML={{
                            __html: bullet
                              .replace(/\*\*(.+?)\*\*/g, "<strong>$1</strong>")
                              .replace(/\*(.+?)\*/g, "<em>$1</em>"),
            ...
            {showContentIdeas && contentIdeas.length > 0 && (
              <>
                ...
                <div className="mt-12 mb-8 pb-6 border-t-2 border-slate-300 pt-8">
                  <h2 className="text-2xl font-bold text-coral-700">
                    PART 2: 10 SPECIFIC CONTENT IDEAS
                  </h2>
                </div>

                {/* Content Ideas List */}
                <div className="space-y-6">
                  {(isEditMode ? editedIdeas : contentIdeas).map(
                    (idea, index) => (
                      <div
                        key={index}
                        className="pb-6 border-b border-slate-300 last:border-b-0"
                        ...
                        {isEditMode && editedIdeas ? (
                          <div className="space-y-3">
                            ...
                            <Textarea
                              ...
                              onChange={(e) => {
                                ...
                                newIdeas[index] = {
                                  ...newIdeas[index],
                                  emotionalIntention: e.target.value,
                                };
                                setEditedIdeas(newIdeas);
                              }}
                          ...
                          <>
                            ...
                            <p className="text-base text-slate-900 mb-2">
                              <span className="font-bold">Core Message:</span>{" "}
                              {idea.coreMessage}
                            </p>
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-slate-600">
          Answer a few questions about your content preferences and we'll
          generate personalized content pillars and ideas for you.
        </p>
      </div>
            ...
            <CardContent className="space-y-6">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>
                      How often can you commit to posting content on one core
                      platform every week?
                    </FormLabel>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>
                      What will be your one core platform you focus on?
                    </FormLabel>
                    <Select onValueChange={field.onChange} value={field.value}>
                      <FormControl>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>
                      What will be your consistent form of weekly content
                      creation? (Podcast, Youtube, Social video series)
                    </FormLabel>
                    <Select onValueChange={field.onChange} value={field.value}>
                      <FormControl>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>
                        1. How do you want people to feel when they interact
                        with your content?
                      </FormLabel>
                      <FormControl>
                        <Textarea
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>
                        2. What do you NOT want people to feel?
                      </FormLabel>
                      <FormControl>
                        <Textarea
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>
                      4. What are the 3–4 biggest themes or truths your audience
                      needs to understand before they're ready to buy from you?
                    </FormLabel>
                    <FormControl>
                      <Textarea
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>
                      5. What common problems, mistakes, or myths do you want to
                      call out and address?
                    </FormLabel>
                    <FormControl>
                      <Textarea
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>
                      6. What values or beliefs do you want to consistently
                      reinforce in your content?
                    </FormLabel>
                    <FormControl>
                      <Textarea
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>
                      7. What are 3-5 contrarian or disruptive takes you have in
                      your niche?
                    </FormLabel>
                    <FormControl>
                      <Textarea
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>
                      8. What's a quick tip, tool, or framework that would help
                      your audience take action today?
                    </FormLabel>
                    <FormControl>
                      <Textarea
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>
                      9. What's a common question or objection your audience
                      has?
                    </FormLabel>
                    <FormControl>
                      <Textarea
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>
                      10. What's a belief shift your audience needs to make to
                      move closer to working with and trusting you?
                    </FormLabel>
                    <FormControl>
                      <Textarea
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>
                      11. What truth or belief do you want to shout from the
                      rooftops that scares you to say, but would set your people
                      free?
                    </FormLabel>
                    <FormControl>
                      <Textarea
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>
                      12. If your audience remembered only one thing from your
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>
                      13. How can you say this in a way that feels authentic to
                      your voice (not generic)?
                    </FormLabel>
                    <FormControl>
                      <Textarea
