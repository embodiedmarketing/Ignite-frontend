...
function parseMarkdownToHTML(markdown: string): string {
  return (
    markdown
      ...
      .replace(
        /^✅ \*\*(.*?)\*\* - (.*$)/gm,
        '<div class="sales-benefit"><span class="checkmark">✅</span><strong>$1</strong> - $2</div>'
      )
      // Module formatting
      .replace(
        /^\*\*(Module \d+:.*?)\*\* - (.*$)/gm,
        '<div class="sales-module"><strong>$1</strong> - $2</div>'
      )
      // Power moves formatting
      .replace(
        /^\*\*(POWER MOVE #\d+:.*?)\*\*/gm,
        '<h4 class="sales-power-move">$1</h4>'
      )
      // Line breaks
...
export default function SimplifiedSalesPageGenerator({
  userId,
}: SimplifiedSalesPageGeneratorProps) {
  ...
  const {
    messagingStrategy: dataMessagingStrategy,
    offerOutline: dataOfferOutline,
  ...
  const deleteAllDrafts = () => {
    try {
      ...
      toast({
        ...
        description:
          "Sales page generator has been reset. You can now generate a fresh sales page.",
      });
    } catch (error) {
      toast({
      ...
      <Card className="border-2 border-yellow-200">
        ...
        <CardContent>
          <p className="text-yellow-700 mb-4">
            To generate your sales page, please complete your messaging strategy
            and offer creation workbook sections first.
          </p>
          <div className="text-sm text-yellow-600">
            <p>
              <strong>Step 1:</strong> Complete "Your Messaging" section
            </p>
            <p>
              <strong>Step 2:</strong> Complete "Create Your Offer" section
            </p>
          </div>
        </CardContent>
      </Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="bg-blue-50 p-3 rounded-lg text-sm">
              ...
              <div className="space-y-1 text-blue-600">
                <p>
                  ✓ Messaging Strategy:{" "}
                  {messagingStrategy?.content ? "Loaded" : "Not available"}
                </p>
                <p>
                  ✓ Offer Outline:{" "}
                  {offerOutline?.content ? "Loaded" : "Not available"}
                </p>
              </div>
            </div>
              ...
              {missingElements.length > 0 && (
                <Alert className="border-orange-200 bg-orange-50 mb-4">
                  ...
                  <AlertDescription className="text-orange-700">
                    <strong>Improve your sales page:</strong> Complete the
                    highlighted sections below for maximum impact.
                  </AlertDescription>
                </Alert>
              )}
                    ...
                    <SalesPageInputField
                      ...
                      value={
                        salesPageInputs[
                          element.field
                            .toLowerCase()
                            .replace(/\s+/g, "") as keyof SalesPageInputs
                        ] || ""
                      }
                      onChange={(value) =>
                        updateInput(
                      ...
                      placeholder={
                        ...
                        `Add your ${element.field.toLowerCase()}...`
                      }
                      userId={userId}
