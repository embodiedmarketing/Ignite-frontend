...
import { Trash2, Edit3, Plus, Calendar, User, Clock, FileText, ChevronDown, ChevronUp, Upload, File, Loader2 } from "lucide-react";
import { useToast } from "@/hooks/use-toast";
import { apiRequest } from "@/services/queryClient";
import type { IcaInterviewTranscript, InsertIcaInterviewTranscript } from "@shared/schema";
...
export function InterviewTranscriptManager({ userId }: InterviewTranscriptManagerProps) {
  const [isCreateDialogOpen, setIsCreateDialogOpen] = useState(false);
  const [editingTranscript, setEditingTranscript] = useState<IcaInterviewTranscript | null>(null);
  ...
  useEffect(() => {
    ...
    const savedFileName = localStorage.getItem(`transcript-draft-filename-${userId}`);
  ...
  useEffect(() => {
    if (newTranscript.title || newTranscript.customerName || newTranscript.interviewDate || newTranscript.rawTranscript) {
      localStorage.setItem(`transcript-draft-${userId}`, JSON.stringify(newTranscript));
  ...
  useEffect(() => {
    if (uploadedFileName) {
      localStorage.setItem(`transcript-draft-filename-${userId}`, uploadedFileName);
    }
  }, [uploadedFileName, userId]);

  const { data: transcripts = [], isLoading } = useQuery<IcaInterviewTranscript[]>({
  ...
  useEffect(() => {
    ...
    if (stuckTranscripts.length > 0) {
      console.log(`🔧 Found ${stuckTranscripts.length} stuck transcripts - resetting to draft`);
      
      // Reset each stuck transcript to draft
      stuckTranscripts.forEach(async (transcript: IcaInterviewTranscript) => {
        try {
          await apiRequest('PUT', `/api/interview-transcripts/${transcript.id}`, {
            status: 'draft'
          });
          console.log(`✅ Reset stuck transcript ${transcript.id} (${transcript.title}) to draft`);
        } catch (error) {
          console.error(`❌ Failed to reset transcript ${transcript.id}:`, error);
        }
      });

      // Refresh the list after a short delay
      setTimeout(() => {
        queryClient.invalidateQueries({ queryKey: [`/api/interview-transcripts/user/${userId}`] });
      }, 1000);
    }
  ...
  const createMutation = useMutation({
    mutationFn: async (data: InsertIcaInterviewTranscript) => {
      const response = await apiRequest('POST', `/api/interview-transcripts`, data);
    ...
    onSuccess: async (createdTranscript, variables) => {
      ...
      try {
        const transcriptMarkdown = `# ${variables.title}\n\n**Customer:** ${variables.customerName || 'Not specified'}\n**Date:** ${variables.interviewDate ? new Date(variables.interviewDate).toLocaleDateString() : 'Not specified'}\n\n---\n\n## Transcript\n\n${variables.rawTranscript}`;
      ...
      setNewTranscript({ title: '', customerName: '', interviewDate: '', rawTranscript: '' });
      setUploadedFileName('');
      
      // Clear localStorage draft after successful save
      localStorage.removeItem(`transcript-draft-${userId}`);
      localStorage.removeItem(`transcript-draft-filename-${userId}`);
      console.log('Cleared transcript draft from localStorage after successful save');
      
      toast({ title: "Interview transcript saved and added to Resources" });
    },
    onError: () => {
      toast({ title: "Failed to save interview transcript", variant: "destructive" });
    },
  });

  const intelligentProcessingMutation = useMutation({
    mutationFn: async ({ transcript, existingMessagingStrategy, transcriptId }: { 
      transcript: string; 
      existingMessagingStrategy: Record<string, string>;
      transcriptId: number | null;
    }) => {
      console.log(`🚀 Starting intelligent processing for transcript ${transcriptId || 'batch'}`);
      
      // First update status to processing (only for individual transcripts)
      if (transcriptId) {
        try {
          await apiRequest('PUT', `/api/interview-transcripts/${transcriptId}`, {
            status: 'processing'
          });
          queryClient.invalidateQueries({ queryKey: [`/api/interview-transcripts/user/${userId}`] });
          console.log(`✅ Updated transcript ${transcriptId} status to 'processing'`);
        } catch (statusError) {
          console.error(`❌ Failed to update status to 'processing':`, statusError);
          throw new Error(`Failed to update transcript status: ${statusError instanceof Error ? statusError.message : 'Unknown error'}`);
        }
      }

      // Then process with AI (with extended timeout for large transcripts)
      try {
        console.log(`🤖 Calling AI processing endpoint with 120 second timeout...`);
        ...
        if (!response.ok) {
          ...
          throw new Error(`AI processing request failed: ${response.status} ${errorText}`);
        }
        ...
        if (aiError instanceof Error && aiError.name === 'AbortError') {
          throw new Error('AI processing timed out after 120 seconds. Please try again with a shorter transcript.');
        }
    ...
    onSuccess: async ({ aiResponse, transcriptId }) => {
      ...
      if (transcriptId) {
        try {
          ...
          queryClient.invalidateQueries({ queryKey: [`/api/interview-transcripts/user/${userId}`] });
          console.log(`✅ Successfully updated transcript ${transcriptId} status to 'processed'`);
        } catch (statusUpdateError) {
          console.error(`❌ Failed to update transcript ${transcriptId} status to 'processed':`, statusUpdateError);
          // Still allow the rest of the processing to continue even if status update fails
          toast({
            title: "Status update failed",
            description: "Interview processed successfully but status update failed. Please refresh the page.",
            variant: "destructive"
          });
        }
      }
      ...
      const messagingUpdates = messagingUpdatesResponse?.updates || messagingUpdatesResponse;
      ...
      if (messagingUpdates && Object.keys(messagingUpdates).length > 0) {
        ...
        const existingNotes = JSON.parse(localStorage.getItem(`interview-notes-${userId}`) || '{}');
        ...
        localStorage.setItem(`interview-notes-${userId}`, JSON.stringify(updatedNotes));

        // Save to database as well
        try {
          const savePromises = Object.entries(messagingUpdates).map(([noteKey, content]) => 
          ...
          console.log(`Successfully saved ${Object.keys(messagingUpdates).length} interview insights to database`);
        } catch (error) {
          console.error('Failed to save interview insights to database:', error);
        }

        // Verify save
        const savedNotes = JSON.parse(localStorage.getItem(`interview-notes-${userId}`) || '{}');
        ...
        const truncationMessage = wasTruncated ? " (Some content was truncated due to length)" : "";
        
        toast({ 
          title: isProcessingAll ? "All interviews processed!" : "Interview processed!",
          description: isProcessingAll 
            ...
            : `Intelligently processed interview insights and updated ${Object.keys(messagingUpdates).length} questions.${truncationMessage}`
        });
        ...
        window.dispatchEvent(new CustomEvent('interviewNotesUpdated', { 
          detail: { messagingUpdates, wasTruncated, shouldUpdateTextAreas: !transcriptId }
        }));
      } else {
    ...
    onError: async (error: any, variables) => {
      ...
      if (isNetworkError) {
        console.log("🔍 Network error detected - checking if processing actually succeeded...");
        ...
        try {
          ...
          const transcriptsResponse = await fetch(`/api/interview-transcripts/user/${userId}`, {
            credentials: 'include'
          });
          
          if (transcriptsResponse.ok) {
            const latestTranscripts = await transcriptsResponse.json();
            const processingCount = latestTranscripts.filter((t: any) => t.status === 'processing').length;
            
            console.log(`📊 Status check: ${processingCount} transcripts still processing`);
            
            // If no transcripts are processing, the job actually completed!
            if (processingCount === 0) {
              console.log("✅ RECOVERY: Processing actually completed despite network error!");
              
              // Refetch interview notes to update UI
              queryClient.invalidateQueries({ queryKey: [`/api/interview-transcripts/user/${userId}`] });
              queryClient.invalidateQueries({ queryKey: ['/api/interview-notes'] });
              
              // Trigger UI update
              window.dispatchEvent(new CustomEvent('interviewNotesUpdated', { 
                detail: { messagingUpdates: {}, wasTruncated: false, shouldUpdateTextAreas: true }
              }));
              
              toast({ 
                title: "Processing completed successfully!", 
                description: "Your interview insights have been saved. Connection was lost but the processing finished."
              });
      ...
      if (variables.transcriptId) {
        try {
          console.log(`🔄 Reverting transcript ${variables.transcriptId} status to 'draft'`);
          await apiRequest('PUT', `/api/interview-transcripts/${variables.transcriptId}`, {
            status: 'draft'
          });
          queryClient.invalidateQueries({ queryKey: [`/api/interview-transcripts/user/${userId}`] });
  ...
  const triggerIntelligentProcessing = (transcript: string, transcriptId: number) => {
    console.log("Triggering intelligent processing with transcript:", transcript?.substring(0, 100) + "...");
    // Get existing messaging strategy from localStorage
    const existingMessagingStrategy = JSON.parse(localStorage.getItem(`step-1-responses-${userId}`) || '{}');
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setNewTranscript(prev => ({ ...prev, rawTranscript: data.transcriptText }));
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: number; data: Partial<IcaInterviewTranscript> }) => {
    ...
    onSuccess: (_, variables) => {
      ...
      if (wasProcessed) {
        toast({ title: "Transcript updated - Process with AI button now available" });
      } else {
        toast({ title: "Interview transcript updated successfully" });
      }
    },
    onError: () => {
      toast({ title: "Failed to update interview transcript", variant: "destructive" });
    },
  });
  ...
  const deleteMutation = useMutation({
    ...
    onError: () => {
      toast({ title: "Failed to delete interview transcript", variant: "destructive" });
    },
  });
  ...
  if (isLoading) {
    return <div className="text-center py-8">Loading interview transcripts...</div>;
  }
          ...
          <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
            ...
            <DialogHeader className="pb-2">
              <div className="flex items-center justify-between">
                <DialogTitle className="text-2xl font-semibold text-[hsl(217,42%,14%)]">Add Interview Transcript</DialogTitle>
                {(newTranscript.title || newTranscript.customerName || newTranscript.interviewDate || newTranscript.rawTranscript) && (
            ...
            <div className="space-y-5 py-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <Label htmlFor="title" className="text-sm font-semibold text-[hsl(217,42%,14%)]">
                    Interview Title
                  </Label>
                  <Input
                    id="title"
                    ...
                    onChange={(e) => setNewTranscript(prev => ({ ...prev, title: e.target.value }))}
                    className="bg-white border border-[hsl(35,20%,85%)] focus-visible:ring-1 focus-visible:ring-[hsl(213,81%,59%)] text-base"
                  />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="customerName" className="text-sm font-semibold text-[hsl(217,42%,14%)]">
                    Customer Name
                  </Label>
                  <Input
                    id="customerName"
                    ...
                    onChange={(e) => setNewTranscript(prev => ({ ...prev, customerName: e.target.value }))}
                    className="bg-white border border-[hsl(35,20%,85%)] focus-visible:ring-1 focus-visible:ring-[hsl(213,81%,59%)] text-base"
                  />
                </div>
              </div>
              ...
              <div className="space-y-2">
                <Label htmlFor="interviewDate" className="text-sm font-semibold text-[hsl(217,42%,14%)]">
                  Interview Date
                </Label>
                <Input
                  id="interviewDate"
                  ...
                  onChange={(e) => setNewTranscript(prev => ({ ...prev, interviewDate: e.target.value }))}
              ...
              <div className="space-y-3 pt-2">
                <Label className="text-base font-semibold text-[hsl(217,42%,14%)]">Upload Transcript</Label>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div className="space-y-2">
                      ...
                      >
                        <div className="text-center">
                          ...
                          <span className="text-sm font-medium text-[hsl(217,42%,14%)]">Click to upload</span>
                        </div>
                      </label>
                      {uploadedFileName && (
                  ...
                  <div className="space-y-2">
                    ...
                    <Textarea
                      ...
                      onChange={(e) => setNewTranscript(prev => ({ ...prev, rawTranscript: e.target.value }))}
                      rows={5}
                      spellCheck={true}
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-[hsl(35,20%,85%)]">
                ...
                <Button
                  ...
                  disabled={!newTranscript.rawTranscript.trim() || createMutation.isPending}
        ...
        <Card>
          <CardContent className="text-center py-8">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              Start organizing your customer interviews by adding your first transcript
            </p>
              ...
              <CardHeader className="pb-3">
                <div className="flex justify-between items-start">
                  ...
                  <div className="flex gap-2">
                    {(transcript.status === 'draft' || transcript.status === 'updated') && (
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => triggerIntelligentProcessing(transcript.rawTranscript, transcript.id)}
                  ...
                  <CardContent className="pt-0">
                    {transcript.notes && (
                      <div className="mb-4">
                        ...
                        <p className="text-sm text-muted-foreground mt-1">{transcript.notes}</p>
                      </div>
                    )}
        ...
        <Dialog open={!!editingTranscript} onOpenChange={() => setEditingTranscript(null)}>
          <DialogContent className="max-w-2xl">
            ...
            <InterviewTranscriptForm
              ...
              onSubmit={(data) => updateMutation.mutate({ id: editingTranscript.id, data })}
...
function InterviewTranscriptForm({ userId, initialData, onSubmit, isLoading }: InterviewTranscriptFormProps) {
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div>
          ...
          <Select value={formData.platform} onValueChange={(value) => setFormData({ ...formData, platform: value })} disabled={isFormDisabled}>
            <SelectTrigger>
      ...
      <div>
        ...
        <Textarea
          ...
          onChange={(e) => setFormData({ ...formData, rawTranscript: e.target.value })}
      ...
      <div className="flex justify-end gap-3 pt-4">
        <Button type="submit" disabled={isFormDisabled}>
          {isLoading ? "Saving..." : isProcessing ? "Processing..." : initialData ? "Update Transcript" : "Save Transcript"}
        </Button>
        {isProcessing && (
