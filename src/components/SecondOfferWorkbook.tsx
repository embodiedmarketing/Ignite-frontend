...
export default function SecondOfferWorkbook({
  ...
  const [expandingPrompts, setExpandingPrompts] =
    useState<ExpandingPromptsState>({});
  const [analyzingPrompts, setAnalyzingPrompts] =
    useState<AnalyzingPromptsState>({});
  ...
  useEffect(() => {
    if (responses && !isLoadingResponses && Object.keys(responses).length > 0) {
      console.log(
        "[MANUAL SAVE] SecondOffer - Loading database responses into local state:",
        responses
      );
  ...
  const isSectionCompleted = (sectionTitle: string): boolean => {
    ...
    console.log(
      `[COMPLETION CHECK] Section "${sectionTitle}": ${isCompleted}`,
      {
        stepNumber: 2,
        userId: parseInt(userId),
        offerNumber: 2,
        availableCompletions: completions.filter(
          (c) => c.stepNumber === 2 && c.offerNumber === 2
        ),
      }
    );
    return isCompleted;
  ...
  const getSectionQuestionKeys = (sectionTitle: string): string[] => {
    ...
    return section.prompts.map((prompt: any) => {
      const questionText =
        typeof prompt === "object" ? prompt.question || String(prompt) : prompt;
  ...
  const checkSectionAutoCompletion = async (sectionTitle: string) => {
    ...
    if (!questionKeys || questionKeys.length === 0) {
      console.log(
        `[AUTO-COMPLETION] No questions found for section: ${sectionTitle}`
      );
      return;
    }
    ...
    const completedQuestions = questionKeys.filter((key) => {
      ...
      console.log(
        `[AUTO-COMPLETION] ${key}: ${response.length} chars, complete: ${isComplete}`
      );
      return isComplete;
    });
    ...
    console.log(
      `[AUTO-COMPLETION] Section ${sectionTitle}: ${completedQuestions.length}/${questionKeys.length} complete, currently marked: ${isCurrentlyCompleted}, should be: ${shouldBeCompleted}`
    );

    if (shouldBeCompleted && !isCurrentlyCompleted) {
      try {
        console.log(
          `[AUTO-COMPLETION] Auto-completing section: ${sectionTitle}`
        );
  ...
  const { data: completions = [], isLoading: isLoadingCompletions } =
    useSectionCompletions(parseInt(userId));
  ...
  const secondOfferOutline = secondOfferOutlines.find(
    (outline: any) => outline.offerNumber === 2
  );
  ...
  const generateSecondOfferOutlineMutation = useMutation({
    ...
    onSuccess: (data) => {
      toast({
        ...
        description:
          "Your comprehensive second offer outline has been created successfully.",
        duration: 4000,
      });
    ...
    onError: (error: any) => {
      toast({
        ...
        description:
          error.message ||
          "Failed to generate second offer outline. Please try again.",
        variant: "destructive",
        duration: 4000,
      });
    },
  ...
  const FeedbackLegend = () => (
    <div
      className="mb-4 p-3 bg-slate-50 border border-slate-200 rounded-lg"
      data-testid="feedback-legend"
    >
  ...
  const handleResponseChange = (
    ...
    console.log(
      `[HYBRID SAVE] SecondOffer HandleResponseChange ${promptKey}:`,
      {
  ...
  const getSectionResponseCount = (sectionTitle: string): number => {
    const sectionQuestions =
      stepContent?.interactivePrompts?.sections?.find(
        (s: any) => s.title === sectionTitle
      )?.prompts || [];
    ...
    return sectionQuestions.filter((prompt: any) => {
      ...
      console.log(
        `[RESPONSE COUNT] ${promptKey}: ${value.length} chars, counted: ${hasResponse}`
      );
      return hasResponse;
    }).length;
  };
  ...
  const analyzeFeedback = async (
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_BASE_URL}/api/ai-feedback`,
        {
          method: "POST",
  ...
  const expandWithAI = async (
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_BASE_URL}/api/interactive-coaching`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      if (data && data.interactivePrompts?.length > 0) {
        toast({
          ...
          description:
            "Review the suggestions below and click to add any that enhance your response.",
        });
      } else {
        toast({
          title: "Coaching Available",
          description:
            data.feedback || "AI coach provided feedback on your response.",
          variant: "default",
        });
      }
    } catch (error) {
    ...
    return (
      <span
        className={`text-xs font-medium ${
          feedbackColors[feedback as keyof typeof feedbackColors]
        }`}
      >
      ...
      <Card>
        <CardContent className="text-center py-12">
          <h3 className="text-lg font-semibold text-slate-900 mb-2">
            Loading Second Offer Content...
          </h3>
          <p className="text-slate-600">
            Please wait while we load the workbook structure.
          </p>
        </CardContent>
      </Card>
    );
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="border-coral-200 bg-gradient-to-r from-coral-50 to-indigo-50">
        <CardHeader>
          ...
          <p className="text-sm text-coral-700 mb-2">
            Create a comprehensive offer outline from your second offer creation
            responses and messaging strategy.
          </p>
          <p className="text-xs text-slate-600 bg-slate-50 p-2 rounded border">
            <strong>Note:</strong> You need at least 60% completion to generate
            your second offer outline. Complete more questions in the sections
            below, focusing on Offer Foundation, Structure & Delivery, and
            Pricing & Positioning.
          </p>
        </CardHeader>
        <CardContent>
        ...
        <Card className="border-green-200 bg-gradient-to-r from-green-50 to-emerald-50">
          <CardHeader>
            ...
            <p className="text-sm text-green-700">
              Your comprehensive second offer outline has been generated and is
              ready for use.
            </p>
          </CardHeader>
          <CardContent>
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  toast({
                    ...
                    description: "Your second offer outline has been copied.",
                  });
                }}
              >
      ...
      {(() => {
        ...
        const isFinallyCompleted = isSectionCompleted(
          "Offer Presentation - Second Offer"
        );
        ...
        console.log("[FINAL COMPLETION DEBUG]", {
          ...
          allCompletions: completions.filter(
            (c) => c.stepNumber === 2 && c.offerNumber === 2
          ),
        });
            ...
            <Card className="border-green-200 bg-green-50">
              <CardContent className="text-center py-6">
                ...
                <h3 className="text-lg font-semibold text-green-900 mb-2">
                  Second Offer Presentation Complete!
                </h3>
                <p className="text-green-700 mb-4">
                  Congratulations! You've completed all sections of your second
                  offer creation.
                </p>
                <Button
                  onClick={async (e) => {
                    ...
                    try {
                      ...
                      const finalSectionTitle =
                        "Offer Presentation - Second Offer"; // ← Match other sections
                      console.log(
                        `[FINAL COMPLETE] Marking final completion:`,
                        {
                      ...
                      toast({
                        ...
                        description:
                          "Your second offer presentation is now complete!",
                        duration: 3000,
                      });
                    } catch (error) {
                      console.error(
                        "[FINAL COMPLETE] Failed to mark final completion:",
                        error
                      );
                      toast({
                        title: "Completion Failed",
                        description:
                          "Failed to mark offer as complete. Please try again.",
                        variant: "destructive",
                        duration: 4000,
                      });
                    }
                ...
                >
                  {markSectionComplete.isPending
                    ? "Completing..."
                    : "Complete Offer Presentation - Second Offer"}
                </Button>
              </CardContent>
            </Card>
          );
      ...
      {stepContent.interactivePrompts.sections.map(
          ...
          return (
            <div key={sectionIndex} className="space-y-4">
                    ...
                    <div className="flex items-center space-x-3">
                      <div className="flex items-center space-x-1">
                        ...
                        <span className="text-xs text-slate-600">
                          {getSectionResponseCount(section.title)} of{" "}
                          {section.prompts.length} completed
                        </span>
                      </div>
                    </div>
                  </div>
                        ...
                        {messagingStrategyContent && (
                          <div className="bg-coral-50 border border-coral-200 rounded-lg p-3 space-y-2">
                              ...
                              <Button
                                ...
                                onClick={() => {
                                  const hasExistingContent = !!(
                                    getCurrentValue(promptKey) &&
                                    getCurrentValue(promptKey)!.trim()
                                  );
                            ...
                            <p className="text-xs text-coral-600">
                              💡 You can continuously edit and refine your
                              response in the text area below
                            </p>
                          </div>
                        )}
                            ...
                            <div className="flex items-center space-x-2">
                              <SaveButton
                                onSave={async () => {
                                  ...
                                  console.log(
                                    "[SECOND OFFER] Save button triggered:",
                                    {
                                      promptKey,
                                      currentValue:
                                      ...
                                      isDirty:
                                        unsavedChanges.isDirty(promptKey),
                                    }
                                  );
                          ...
                          {(() => {
                            ...
                            return (
                              <div className="mt-4 bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                                ...
                                {coachingData.interactivePrompts &&
                                  coachingData.interactivePrompts.length >
                                    0 && (
                                    <div className="space-y-2">
                                      <h5 className="text-sm font-medium text-yellow-800">
                                        Improved Versions:
                                      </h5>
                                      {coachingData.interactivePrompts.map(
                                            ...
                                            <Button
                                              ...
                                              onClick={() => {
                                                ...
                                                toast({
                                                  ...
                                                  description:
                                                    "AI suggestion has been added to your response.",
                                                });
                                              }}
                                ...
                                {coachingData.examples &&
                                  coachingData.examples.length > 0 && (
                                    <div className="mt-3">
                                      <h5 className="text-sm font-medium text-yellow-800 mb-2">
                                        Examples:
                                      </h5>
                                      <ul className="text-sm text-yellow-700 space-y-1">
                                        {coachingData.examples.map(
              ...
              {(() => {
                ...
                const isSecondOfferCompleted = completions.some(
                  (c) =>
                    c.sectionTitle === sectionTitle &&
                    c.stepNumber === 2 &&
                    c.userId === parseInt(userId) &&
                ...
                console.log(
                  `[COMPLETION RENDER] Section "${sectionTitle}": isCompleted=${isSecondOfferCompleted}, offerNumber=2, stepNumber=2`
                );
                    ...
                    <CardContent className="text-center py-6">
                      ...
                      <Button
                        onClick={async (e) => {
                          ...
                          try {
                            console.log(
                              `[MANUAL COMPLETE] Marking section complete:`,
                              {
                                sectionTitle,
                                stepNumber: 2,
                                userId: parseInt(userId),
                            ...
                            console.error(
                              "[MANUAL COMPLETE] Failed to mark section complete:",
                              error
                            );
                            toast({
                              title: "Completion Failed",
                      ...
                      >
                        {markSectionComplete.isPending
                          ? "Completing..."
                          : `Complete ${sectionTitle} Section - Second Offer`}
                      </Button>
                    </CardContent>
                  </Card>
                ) : (
                    ...
                    <CardContent className="text-center py-4">
                      <div className="flex items-center justify-center space-x-2">
                        ...
                        <span className="text-green-800 font-medium">
                          {sectionTitle} Section Completed - Second Offer!
                        </span>
                      </div>
                      <p className="text-green-600 text-sm mt-1">
                        Great work! You can continue editing your responses if
                        needed.
                      </p>
                      <div className="mt-3">
                        <Button
                          ...
                          onClick={async (e) => {
                            ...
                            try {
                              console.log(
                                `[UNMARK COMPLETE] Unmarking section:`,
                                {
                                  sectionTitle,
                                  stepNumber: 2,
                                  userId: parseInt(userId),
                              ...
                              console.error(
                                "[UNMARK COMPLETE] Failed to unmark section:",
                                error
                              );
                              toast({
                                title: "Unmark Failed",
                                description:
                                  "Failed to unmark section. Please try again.",
                                variant: "destructive",
                                duration: 4000,
                              });
                            }
                          ...
                          className="text-sm"
                        >
                          {unmarkSectionComplete.isPending
                            ? "Unmarking..."
                            : "Unmark as Complete"}
                        </Button>
                      </div>
