...
export default function CompletedIgniteDocs() {
  ...
  const [selectedDocument, setSelectedDocument] =
    useState<IGNITEDocument | null>(null);
  ...
  } = useQuery<MessagingStrategy[]>({
    queryKey: ["/api/messaging-strategies/user", user?.id],
    queryFn: async () => {
      const response = await fetch(
        `${import.meta.env.VITE_BASE_URL}/api/messaging-strategies/user/${
          user?.id
        }`,
        {
          credentials: "include",
  ...
  const {
    ...
    error: outlinesError,
  } = useQuery<UserOfferOutline[]>({
    queryKey: ["/api/user-offer-outlines/user", user?.id],
    queryFn: async () => {
      const response = await fetch(
        `${import.meta.env.VITE_BASE_URL}/api/user-offer-outlines/user/${
          user?.id
        }`,
        {
          credentials: "include",
  ...
  const {
    data: igniteDocuments,
    isLoading: igniteDocsLoading,
    error: igniteDocsError,
  } = useQuery<IgniteDocument[]>({
    queryKey: ["/api/ignite-docs/user", user?.id],
    queryFn: async () => {
      const response = await fetch(
        `${import.meta.env.VITE_BASE_URL}/api/ignite-docs/user/${user?.id}`,
        {
          credentials: "include",
        }
      );
  ...
  const updateMessagingStrategyMutation = useMutation({
    mutationFn: async ({ id, content }: { id: number; content: string }) => {
      return await apiRequest("PUT", `/api/messaging-strategies/${id}`, {
        content,
      });
    },
    onSuccess: () => {
  ...
  const updateOfferOutlineMutation = useMutation({
    mutationFn: async ({ id, content }: { id: number; content: string }) => {
      return await apiRequest("PUT", `/api/user-offer-outlines/${id}`, {
        content,
      });
    },
    onSuccess: () => {
  ...
  const updateIgniteDocMutation = useMutation({
    mutationFn: async ({
      ...
      return await apiRequest("PUT", `/api/ignite-docs/${id}`, {
        contentMarkdown,
      });
    },
    onSuccess: () => {
  ...
  const handleSaveEdit = (doc: IGNITEDocument) => {
    if (doc.type === "messaging") {
      updateMessagingStrategyMutation.mutate({
        id: doc.id,
      ...
      updateIgniteDocMutation.mutate({
        id: doc.id,
  ...
  const documents: IGNITEDocument[] = [
    ...
    ...(offerOutlines || [])
      .filter((o) => o.title?.includes("Tripwire"))
      .map((o) => ({
        id: o.id,
    ...
    ...(offerOutlines || [])
      .filter((o) => o.title?.includes("Core"))
      .map((o) => ({
        id: o.id,
        type: "core" as const,
    ...
    ...(igniteDocuments || []).map((d) => ({
      ...
      title:
        d.title ||
        (d.docType === "content_strategy"
          ? "Content Strategy Plan"
          : d.docType === "content_ideas"
          ? "Content Ideas"
          : d.docType === "email_sequence"
          ? "Email Sequence"
          : d.docType === "video_scripts"
          ? "Video Script Generator"
          : d.docType === "launch_registration_funnel"
          ? "Launch Registration Funnel Copy"
          : d.docType === "sales_page"
          ? "Sales Page Copy"
          : d.docType === "launch_email_sequence"
          ? "Launch Email Sequence"
          : d.docType === "funnel_optimization"
          ? "Funnel Optimization Suggestions"
          : d.docType === "interview_transcript"
          ? "Interview Transcript"
          : d.docType === "messaging_strategy"
          ? "Messaging Strategy"
          : "Copy Generation"),
      content: d.contentMarkdown,
      createdAt: d.createdAt,
      editLink:
        d.docType === "lead_generation"
  ...
  const foundationDocs = documents.filter(
    (doc) =>
      doc.type === "messaging" ||
      doc.type === "tripwire" ||
      doc.type === "core" ||
      doc.type === "interview_transcript" ||
      doc.type === "messaging_strategy"
  );
  const contentStrategyDocs = documents.filter(
    (doc) => doc.type === "content_strategy" || doc.type === "content_ideas"
  );
  const resourcesDocs = documents.filter(
    (doc) => doc.type === "lead_generation" || doc.type === "email_sequence"
  );
  const liveLaunchDocs = documents.filter(
    (doc) =>
      doc.type === "launch_registration_funnel" ||
      doc.type === "sales_page" ||
      doc.type === "launch_email_sequence" ||
      doc.type === "video_scripts" ||
      doc.type === "funnel_optimization"
  );
    ...
    lines.forEach((line) => {
      if (line.startsWith("# ")) {
        ...
        elements.push(
          <h3
            key={`h3-${index++}`}
            className="text-sm font-semibold text-slate-700 mt-4 mb-2"
          >
            {line.replace("### ", "")}
          </h3>
        );
        ...
        elements.push(
          <p
            key={`p-${index++}`}
            className="mb-3 leading-relaxed text-slate-700 text-sm"
          >
        ...
        flushList();
        elements.push(
          <hr key={`hr-${index++}`} className="my-6 border-slate-300" />
        );
      } else if (line.trim() === "") {
        flushList();
        ...
        elements.push(
          <p
            key={`text-${index++}`}
            className="mb-2 leading-relaxed text-slate-600 text-sm"
          >
            {line}
          </p>
        );
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-slate-900">
              Completed IGNITE Docs
            </h1>
            <p className="text-slate-600 mt-2">
              Access your completed docs and copy as you move through the IGNITE
              journey. You can view, edit and download each doc!
            </p>
          </div>
                        ...
                        <div className="flex items-center justify-between mb-3">
                          ...
                          <div className="flex items-center gap-2">
                            <Badge
                              ...
                              data-testid={`badge-complete-${doc.id}`}
                            >
                              Complete
                            </Badge>
                            <Button
                            ...
                            {editingDocId === doc.id ? (
                              <>
                                <Button
                                  ...
                                  disabled={
                                    updateMessagingStrategyMutation.isPending ||
                                    updateOfferOutlineMutation.isPending ||
                                    updateIgniteDocMutation.isPending
                                  }
                                >
                                  {updateMessagingStrategyMutation.isPending ||
                                  updateOfferOutlineMutation.isPending ||
                            ...
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent>
                                <DropdownMenuItem
                                  ...
                                  data-testid={`menu-download-docx-${doc.id}`}
                                ...
                                <DropdownMenuItem
                                  ...
                                  data-testid={`menu-download-pdf-${doc.id}`}
                                  className="hover:bg-slate-100 focus:bg-slate-100"
                                >
                                  Download PDF
                                </DropdownMenuItem>
          ...
          <Card>
            <CardContent className="text-center py-12">
              ...
              <p className="text-slate-600 mb-4">
                Complete your messaging strategy, tripwire offer outline, or
                core offer outline to see your documents here.
              </p>
              <Link href="/step/1">
                <Button>Go to Strategic Messaging</Button>
              </Link>
            </CardContent>
          </Card>
        )}
