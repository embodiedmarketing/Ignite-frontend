...
export default function BuildingYourStrategy() {
  ...
  const { activeStrategy, refetch: refetchMessagingStrategy } =
    useMessagingStrategy(userId);
  ...
  useEffect(() => {
    if (offerOutlines && offerOutlines.length > 0) {
      ...
      if (tripwireOutline && tripwireOutline.content) {
        ...
        // Extract Format (type)
        const format = extractField(
          content,
          /\*\*Format \+ Duration:\*\*\s*"?Delivered as\s*([^"]+?)(?:\s*over|\.|")/
        );
        ...
        const coreTransformation = extractField(
          content,
          /\*\*Core Transformation \(Main Promise\):\*\*\s*([^\n]+)/
        );
        ...
        const valueFraming = extractField(
          content,
          /\*\*Value Framing:\*\*\s*"([^"]+)"/
        );
        ...
        const regularPriceMatch = extractField(
          content,
          /\(Normally\s*\$?([^)]+)\)/
        );
        ...
        const problemSolves = extractField(
          content,
          /\*\*Problem This Solves:\*\*\s*([^\n*]+)/
        );
        ...
        const different = extractField(
          content,
          /\*\*What Makes It Different:\*\*\s*([^\n*]+)/
        );
        ...
        const emotionalBenefit = extractField(
          content,
          /\*\*Ultimate Emotional Benefit:\*\*\s*"([^"]+)"/
        );
        ...
        const falseBeliefs = extractField(
          content,
          /\*\*False Beliefs \/ Objections \(and Reframes\):\*\*\s*([^\n*]+)/
        );
        ...
        const guarantee = extractField(
          content,
          /\*\*Guarantee \/ Risk Reversal:\*\*\s*"([^"]+)"/
        );
        ...
        const testimonialsText = extractField(
          content,
          /\*\*Testimonials \/ Social Proof:\*\*\s*([^\n#]+)/
        );
        ...
        const expertise = extractField(
          content,
          /\*\*Expertise \/ Credentials:\*\*\s*([^\n*]+)/
        );
  ...
  useEffect(() => {
    if (activeStrategy && activeStrategy.content) {
      ...
      const positioningSection = extractSection(
        content,
        "UNIQUE POSITIONING FOUNDATION"
      );
      if (positioningSection) {
        // Extract "What Makes You Uniquely Qualified" for authority
        const uniqueQualified = extractField(
          positioningSection,
          /\*\*What Makes You Uniquely Qualified:\*\*\s*([^*]+?)(?=\*\*|$)/s
        );
        ...
        const transformationText = extractField(
          positioningSection,
          /\*\*The Transformation You Create:\*\*\s*([^*]+?)(?=\*\*|---|\n\n##|$)/s
        );
        ...
        const distinctiveApproach = extractField(
          positioningSection,
          /\*\*Your Distinctive Approach:\*\*\s*([^*]+?)(?=\*\*|$)/s
        );
      ...
      const avatarSection = extractSection(
        content,
        "CUSTOMER AVATAR DEEP DIVE"
      );
      if (avatarSection) {
        // Extract pain points/challenges
        const painPointsText = extractField(
          avatarSection,
          /\*\*(?:Deepest Fears|Main Pain Points|Current Challenges):\*\*\s*([^*]+?)(?=\*\*|$)/s
        );
        ...
        const desiresText = extractField(
          avatarSection,
          /\*\*(?:Deepest Desires|What They Want Most|Ultimate Goal):\*\*\s*([^*]+?)(?=\*\*|$)/s
        );
      ...
      if (offerSection) {
        ...
        const mainPromise = extractField(
          offerSection,
          /\*\*(?:Main Promise|Core Transformation|Primary Outcome):\*\*\s*([^*]+?)(?=\*\*|$)/s
        );
        ...
        const problemText = extractField(
          offerSection,
          /\*\*(?:Problem This Solves|Core Problem Addressed):\*\*\s*([^*]+?)(?=\*\*|$)/s
        );
      ...
      if (presentationSection) {
        // Extract objections
        const objectionsText = extractField(
          presentationSection,
          /\*\*(?:Common Objections|Addressing Objections|Overcoming Resistance):\*\*\s*([^*]+?)(?=\*\*|$)/s
        );
        ...
        const socialProofText = extractField(
          presentationSection,
          /\*\*(?:Social Proof|Testimonials|Results):\*\*\s*([^*]+?)(?=\*\*|$)/s
        );
        ...
        // Extract urgency
        const urgencyText = extractField(
          presentationSection,
          /\*\*(?:Creating Urgency|Why Now|Urgency Drivers):\*\*\s*([^*]+?)(?=\*\*|$)/s
        );
      ...
      if (pricingSection) {
        // Extract investment/price
        const investmentText = extractField(
          pricingSection,
          /\*\*(?:Investment|Pricing|Price Point):\*\*\s*\$?([^*\n]+?)(?=\*\*|\n|$)/s
        );
        ...
        const valueText = extractField(
          pricingSection,
          /\*\*(?:Value Positioning|Why This Price|Value Justification):\*\*\s*([^*]+?)(?=\*\*|$)/s
        );
  ...
  useEffect(() => {
    if (igniteDocuments && !generatedEmailSequence) {
      ...
      const emailSequenceDocs = igniteDocuments.filter(
        (doc: any) => doc.docType === "email_sequence"
      );
      ...
      if (emailSequenceDocs.length > 0) {
        ...
        console.log(
          "[EMAIL SEQUENCE] Loading saved email sequence from IGNITE Docs"
        );
  ...
  useEffect(() => {
    if (sectionCompletionsData && sectionCompletionsData.length > 0) {
      console.log(
        "[IMPLEMENTATION STEPS] Loading saved completion states from database"
      );
      ...
      const step4Completions = sectionCompletionsData.filter(
        (c) => c.stepNumber === 4
      );
  ...
  useEffect(() => {
    if (responses && Object.keys(responses).length > 0) {
      ...
      if (responsesJSON !== prevResponsesJSON) {
        ...
        setOvercomingObjections(
          responses["leadgen-overcomingObjections"] || ""
        );
    ...
    try {
      ...
      if (savedToIgnite) {
        toast({
          ...
          description:
            "Your funnel copy has been successfully generated and saved to IGNITE Docs.",
        });
      } else {
        toast({
          title: "Funnel Copy Generated!",
          description:
            "Your funnel copy has been generated. Note: Failed to auto-save to IGNITE Docs.",
        });
      }
    } catch (error) {
  ...
  const handleGenerateEmailSequence = async () => {
    ...
    try {
      ...
      const idealCustomerProfile =
        extractSection(messagingStrategyContent, "CUSTOMER AVATAR DEEP DIVE") ||
      ...
      try {
        ...
        toast({
          title: "Note",
          description:
            "Email sequence generated but could not be saved to IGNITE Docs. You can still export it below.",
          variant: "default",
        });
      }
  ...
  const handleEditEmailSequence = () => {
    ...
    setEditedEmailSequence(
      generatedEmailSequence.map((email) => ({ ...email }))
    );
    setIsEditingEmailSequence(true);
  };

  // Function to update an email in the edited sequence
  const handleUpdateEmailInSequence = (
    index: number,
    field: "subject" | "body",
    value: string
  ) => {
      ...
      <div className="mb-8">
        <div>
          ...
          <p className="text-lg text-slate-600">
            Build your lead generation funnel so it's ready for ads!
          </p>
        </div>
      </div>
            ...
            <Card>
              ...
              <CardContent>
                <div className="prose max-w-none text-slate-700 leading-relaxed space-y-4">
                  <p>
                    We're going to start building your lead gen funnel! In this
                    first step we're building out the funnel and the assets
                    based on the strategy we developed. You'll build your funnel
                    pages using our tools and templates, write your lead gen
                    email sequence using our tool and connect all the tech so
                    everything runs on autopilot.
                  </p>
                  <p>
                    By the end, you'll have a simple, complete funnel designed
                    to attract new leads and drive them to your tripwire offer
                    and eventually your core offer!
                  </p>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent>
                <div className="text-center">
                  ...
                  <p className="text-sm text-slate-600">
                    Depending on your pace, setting up your funnel pages,
                    writing emails, and connecting tech may take a little time.
                  </p>
                </div>
              </CardContent>
            </Card>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <p className="text-slate-700">
                      Generate your funnel copy (opt-in page, tripwire page,
                      confirmation page) using the Copy Generator
                    </p>
                  </div>
                  <div className="flex items-start gap-3">
                    ...
                    <p className="text-slate-700">
                      Plug your copy into our Go High Level funnel templates
                    </p>
                  </div>
                  <div className="flex items-start gap-3">
                    <div className="w-6 h-6 rounded-full bg-green-500 text-white text-sm font-medium flex items-center justify-center flex-shrink-0 mt-0.5">
                    ...
                    <p className="text-slate-700">
                      Generate your lead gen email sequence using the Copy
                      Generator
                    </p>
                  </div>
                  <div className="flex items-start gap-3">
                    ...
                    <p className="text-slate-700">
                      Load emails into Go High Level (or other CRM) and connect
                      automations
                    </p>
                  </div>
                  <div className="flex items-start gap-3">
                    ...
                    <p className="text-slate-700">
                      Set up your tech (forms, tags, payment links, automations)
                    </p>
                  </div>
                  <div className="flex items-start gap-3">
                    ...
                    <p className="text-slate-700">
                      Test your funnel + emails to confirm everything works
                    </p>
                  </div>
                </div>
              </CardContent>
              ...
              <CardContent>
                <div className="space-y-3">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <span className="text-slate-600">
                        {" "}
                        → Use our trainings, checklists, and Go High Level
                        templates so you're never starting from scratch.
                      </span>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <span className="text-slate-600">
                        {" "}
                        → Join coaching calls when you're stuck or
                        overwhelmed—screenshare, ask questions, get unstuck
                        live.
                      </span>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <strong className="text-slate-900">
                        Start with generators (70%)
                      </strong>
                      <span className="text-slate-600">
                        {" "}
                        → Use the Copy Generator as your first draft, then
                        refine headlines, benefits, proof, and CTAs.
                      </span>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <strong className="text-slate-900">
                        Reference your Messaging Strategy
                      </strong>
                      <span className="text-slate-600">
                        {" "}
                        → Ensure every promise and CTA aligns with your core
                        message and what your audience actually wants. Read
                        every word as if they are saying it and make sure it
                        makes sense.
                      </span>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <strong className="text-slate-900">
                        Go deep on the messaging
                      </strong>
                      <span className="text-slate-600">
                        {" "}
                        → Make the opt-in a clear, specific win and the tripwire
                        a true "no-brainer" aligned to your core offer.
                      </span>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <strong className="text-slate-900">
                        Be intentional with the journey
                      </strong>
                      <span className="text-slate-600">
                        {" "}
                        → Remember you're creating an experience for the leads
                        to go from your ad to your opt in to your tripwire offer
                        and into your email sequence. It's all one experience.
                      </span>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <span className="text-slate-600">
                        {" "}
                        → One page at a time. Don't worry about advanced
                        automations until the basics are live.
                      </span>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <span className="text-slate-600">
                        {" "}
                        → Forms, tags, triggers, payment links, mobile views,
                        and email deliverability before you launch.
                      </span>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <span className="text-slate-600">
                        {" "}
                        → Publish, collect data (opt-in rate, tripwire take
                        rate, opens/clicks), then iterate.
                      </span>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <span className="text-slate-600">
                        {" "}
                        → You can't mess this up—as long as you launch, you win.
                      </span>
                    </div>
                  </div>
                </div>
          ...
          <Card>
            ...
            <CardContent>
              <p className="text-lg text-slate-700 mb-4">
                Watch these training videos for a complete breakdown of lead gen
                funnel strategies and technical setup
              </p>
          ...
          <Card>
            ...
            <CardContent>
              <p className="text-slate-700 mb-4">
                Use this tool to generate your funnel copy and email copy.
                Remember this is meant to get you 70% of the way there and you
                will finish the last 30%!
              </p>
                  ...
                  <div className="space-y-8">
                                  ...
                                  <div className="flex gap-2">
                                    {editingPage === "optInPage" ? (
                                      ...
                                      <>
                                        ...
                                        <div
                                          id="export-menu-opt-in"
                                          className="hidden absolute right-0 mt-10 w-48 bg-white border border-slate-200 rounded-md shadow-lg z-10"
                                        >
                                          <button
                                ...
                                <div
                                  ...
                                  onBlur={(e) => {
                                    if (editingPage === "optInPage") {
                                      setEditedContent({
                                        ...editedContent,
                                        optInPage: e.currentTarget.innerHTML,
                                      });
                                    }
                                  }}
                                  dangerouslySetInnerHTML={{
                                    __html:
                                      editingPage === "optInPage"
                                        ? editedContent.optInPage
                                        : generatedCopy.optInPage,
                                  }}
                                ...
                                <div
                                  ...
                                  onBlur={(e) => {
                                    if (editingPage === "tripwirePage") {
                                      setEditedContent({
                                        ...editedContent,
                                        tripwirePage: e.currentTarget.innerHTML,
                                      });
                                    }
                                  }}
                                  dangerouslySetInnerHTML={{
                                    __html:
                                      editingPage === "tripwirePage"
                                        ? editedContent.tripwirePage
                                        : generatedCopy.tripwirePage,
                                  }}
                                ...
                                <div
                                  ...
                                  onBlur={(e) => {
                                    if (editingPage === "checkoutPage") {
                                      setEditedContent({
                                        ...editedContent,
                                        checkoutPage: e.currentTarget.innerHTML,
                                      });
                                    }
                                  }}
                                  dangerouslySetInnerHTML={{
                                    __html:
                                      editingPage === "checkoutPage"
                                        ? editedContent.checkoutPage
                                        : generatedCopy.checkoutPage,
                                  }}
                                    ...
                                    {editingPage === "confirmationPage" ? (
                                      ...
                                      <>
                                        <Button
                                          size="sm"
                                          onClick={() =>
                                            handleEditPage("confirmationPage")
                                          }
                              ...
                              <CardContent>
                                <div
                                  ...
                                  dangerouslySetInnerHTML={{
                                    __html:
                                      editingPage === "confirmationPage"
                                        ? editedContent.confirmationPage
                                        : generatedCopy.confirmationPage,
                                  }}
                    ...
                    <div className="space-y-6">
                      ...
                      <Card>
                        ...
                        <CardContent className="space-y-4">
                          <div className="space-y-3">
                            <Label htmlFor="leadMagnetTitle">
                              1. What is the title or name of your lead magnet?
                            </Label>
                            <Input
                              id="leadMagnetTitle"
                          ...
                          <div className="space-y-3">
                            <Label htmlFor="resourceType">
                              2. What type of resource is it? (e.g. PDF guide,
                              quiz, checklist, template, video training, etc.)
                            </Label>
                            <Input
                              id="resourceType"
                          ...
                          <div className="space-y-3">
                            <Label htmlFor="transformation">
                              ...
                              will someone get from it? The main outcome and use
                              your ICA's language!
                            </Label>
                            <Textarea
                              id="transformation"
                          ...
                          <div className="space-y-3">
                            <Label htmlFor="problemSolved">
                              4. What specific problem does it solve for your
                          ...
                          <div className="space-y-3">
                            <Label htmlFor="urgency">
                              5. Why is this problem urgent or important to
                              solve?
                            </Label>
                            <Textarea
                              id="urgency"
                          ...
                          <div className="space-y-3">
                            <Label htmlFor="uniqueness">
                              6. What's unique about your lead magnet? Think
                              about the outcome and how you help them achieve
                              that outcome, why is that better than alternative
                              pieces of content?
                            </Label>
                            <Textarea
                              id="uniqueness"
                      ...
                      <Card>
                        ...
                        <CardContent className="space-y-4">
                          ...
                          <div className="space-y-3">
                            <Label htmlFor="bulletPoints">
                              4. If your lead magnet had 3 bullet points to sell
                          ...
                          <div className="space-y-3">
                            <Label htmlFor="socialProof">
                              ...
                              you'd use to showcase how great this lead gen
                              offer is?
                            </Label>
                            <Textarea
                              id="socialProof"
                    ...
                    <div className="space-y-6">
                      ...
                      <Card>
                        ...
                        <CardContent className="space-y-4">
                          <div className="space-y-3">
                            <Label htmlFor="tripwireTitle">
                              1. What is the title or name of your tripwire
                              offer?
                            </Label>
                            <Input
                              id="tripwireTitle"
                          ...
                          <div className="space-y-3">
                            <Label htmlFor="tripwireType">
                              2. What type of product is it? (e.g. mini-course,
                              template pack, workshop, audit, starter kit,
                              script library, etc.)
                            </Label>
                            <Input
                              id="tripwireType"
                          ...
                          <div className="space-y-3">
                            <Label htmlFor="tripwireOutcome">
                              3. In one sentence, what's the core outcome/result
                              someone gets from this tripwire? Use your ICA's
                              language.
                            </Label>
                            <Textarea
                              id="tripwireOutcome"
                              ...
                              onChange={(e) =>
                                setTripwireOutcome(e.target.value)
                              }
                              onBlur={(e) =>
                                handleSaveResponse(
                                  "leadgen-tripwireOutcome",
                                  e.target.value
                                )
                              }
                            />
                          ...
                          <div className="space-y-3">
                            <Label htmlFor="tripwireConnection">
                              4. How does this tripwire directly connect to the
                              lead magnet they just opted into? Why is it the
                              next step?
                            </Label>
                            <Textarea
                              ...
                              onChange={(e) =>
                                setTripwireConnection(e.target.value)
                              }
                              onBlur={(e) =>
                                handleSaveResponse(
                                  "leadgen-tripwireConnection",
                                  e.target.value
                                )
                              }
                            />
                          ...
                          <div className="space-y-3">
                            <Label htmlFor="tripwirePrice">
                              5. What's the price point of the tripwire?
                            </Label>
                            <Input
                              id="tripwirePrice"
                          ...
                          <div className="space-y-3">
                            <Label htmlFor="noBrainer">
                              6. Why is this offer a "no-brainer" at that price?
                            </Label>
                            <Textarea
                              id="noBrainer"
                          ...
                          <div className="space-y-3">
                            <Label htmlFor="regularPrice">
                              7. What would you regularly charge for this
                              product?
                            </Label>
                            <Input
                              id="regularPrice"
                      ...
                      <Card>
                        ...
                        <CardContent className="space-y-4">
                          <div className="space-y-3">
                            <Label htmlFor="tripwireProblem">
                              7. What specific problem or pain point does this
                              tripwire solve that your ideal customer feels
                              right now?
                            </Label>
                            <Textarea
                              id="tripwireProblem"
                              ...
                              onChange={(e) =>
                                setTripwireProblem(e.target.value)
                              }
                              onBlur={(e) =>
                                handleSaveResponse(
                                  "leadgen-tripwireProblem",
                                  e.target.value
                                )
                              }
                            />
                          ...
                          <div className="space-y-3">
                            <Label htmlFor="tripwireDifferent">
                              8. What makes this tripwire different or better
                              than alternatives? (Think: speed, simplicity,
                              format, effectiveness, uniqueness.)
                            </Label>
                            <Textarea
                              ...
                              onChange={(e) =>
                                setTripwireDifferent(e.target.value)
                              }
                              onBlur={(e) =>
                                handleSaveResponse(
                                  "leadgen-tripwireDifferent",
                                  e.target.value
                                )
                              }
                            />
                          ...
                          <div className="space-y-3">
                            <Label htmlFor="tripwireWins">
                              9. What quick win(s) will they walk away with
                      ...
                      <Card>
                        ...
                        <CardContent className="space-y-4">
                          <div className="space-y-3">
                            <Label htmlFor="tripwireObjections">
                              11. What objections might someone have before
                              buying?
                            </Label>
                            <Textarea
                              ...
                              onChange={(e) =>
                                setTripwireObjections(e.target.value)
                              }
                              onBlur={(e) =>
                                handleSaveResponse(
                                  "leadgen-tripwireObjections",
                                  e.target.value
                                )
                              }
                            />
                          ...
                          <div className="space-y-3">
                            <Label htmlFor="overcomingObjections">
                              12. How would you answer or overcome those
                              objections?
                            </Label>
                            <Textarea
                              ...
                              onChange={(e) =>
                                setOvercomingObjections(e.target.value)
                              }
                              onBlur={(e) =>
                                handleSaveResponse(
                                  "leadgen-overcomingObjections",
                                  e.target.value
                                )
                              }
                            />
                          ...
                          <div className="space-y-3">
                            <Label htmlFor="riskRemoval">
                              13. What risks are you removing for them? (e.g.
                              money-back guarantee, instant access, done-for-you
                              templates.)
                            </Label>
                            <Textarea
                              id="riskRemoval"
                        ...
                        <CardContent className="space-y-4">
                          <div className="space-y-3">
                            <Label htmlFor="testimonials">
                              14. Do you have any testimonials, screenshots, or
                              results you can include to show this works? It's
                              okay if they are not for this exact product.
                            </Label>
                            <Textarea
                              id="testimonials"
                          ...
                          <div className="space-y-3">
                            <Label htmlFor="authority">
                              15. Do you have any authority, experience, or
                              credentials that increase trust in this offer and
                              should be mentioned?
                            </Label>
                            <Textarea
                              id="authority"
                    ...
                    <div className="flex flex-col items-center gap-4">
                      ...
                      <p className="text-sm text-slate-600">
                        Generate all 4 pages: Opt-in, Tripwire, Checkout &
                        Confirmation
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-6">
                    ...
                    {generatedEmailSequence && (
                      ...
                      >
                        ...
                        <Dialog
                          open={isEditingEmailSequence}
                          onOpenChange={(open) =>
                            !open && handleCancelEmailSequenceEdit()
                          }
                        >
                          <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
                            <DialogHeader>
                              ...
                              <DialogDescription>
                                Make changes to your email sequence. Edit
                                subject lines and body content for each email.
                              </DialogDescription>
                            </DialogHeader>
                                ...
                                >
                                  ...
                                  <CardContent className="pt-4 space-y-3">
                                    <div className="space-y-2">
                                      <Label
                                        htmlFor={`subject-${email.emailNumber}`}
                                    ...
                                    <div className="space-y-2">
                                      <Label
                                        htmlFor={`body-${email.emailNumber}`}
                                        className="text-sm font-medium"
                                      >
                                        Email Body
                                      </Label>
                                      ...
                                      <p className="text-xs text-slate-500">
                                        Tip: Use double line breaks (press Enter
                                        twice) to create new paragraphs.
                                      </p>
                                    </div>
                                  </CardContent>
                                </Card>
                    ...
                    <Card>
                      ...
                      <CardContent className="space-y-4">
                        <div className="space-y-3">
                          <Label htmlFor="coreBeliefShift">
                            1. What's one core belief, mindset shift, or "aha
                            moment" you want your audience to have while reading
                            this sequence?
                          </Label>
                          <Textarea
                            id="coreBeliefShift"
                        ...
                        <div className="space-y-3">
                          <Label htmlFor="objectionsDoubts">
                            2. What common objections, doubts, or limiting
                            beliefs do you want to gently overcome in these
                            emails?
                          </Label>
                          <Textarea
                            id="objectionsDoubts"
                            ...
                            onChange={(e) =>
                              setObjectionsDoubts(e.target.value)
                            }
                            onBlur={(e) =>
                              handleSaveResponse(
                                "leadgen-objectionsDoubts",
                                e.target.value
                              )
                            }
                          />
                        ...
                        <div className="space-y-3">
                          <Label htmlFor="storiesExamples">
                            3. What stories, personal experiences, or client
                            examples could help you build connection and trust
                            in an organic way?
                          </Label>
                          <Textarea
                            id="storiesExamples"
                    ...
                    <Card>
                      ...
                      <CardContent className="space-y-4">
                        <div className="space-y-3">
                          <Label htmlFor="contentHighlight">
                            4. What other pieces of content do you want to
                            highlight in this sequence? (e.g. podcast episodes,
                            social media channels, blog posts, YouTube videos —
                            include links.)
                          </Label>
                          <Textarea
                            id="contentHighlight"
                            ...
                            onChange={(e) =>
                              setContentHighlight(e.target.value)
                            }
                            onBlur={(e) =>
                              handleSaveResponse(
                                "leadgen-contentHighlight",
                                e.target.value
                              )
                            }
                          />
                        ...
                        <div className="space-y-3">
                          <Label htmlFor="contentOrder">
                            5. Is there a specific order in which you'd like
                            subscribers to see this content (or should AI decide
                            the flow)?
                          </Label>
                          <Textarea
                            id="contentOrder"
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            ...
                            <h3 className="font-semibold text-slate-900 text-left">
                              Create Your Lead Gen Funnel Copy
                            </h3>
                          </div>
                    ...
                    <CollapsibleContent>
                      <CardContent className="pt-0">
                        <p className="text-slate-600">
                          Answer the Copy Generator questions in as much detail
                          as you can to generate your copy! Take that copy and
                          finalize it. It's meant to get you 70% of the way
                          there and you finish the last 30%! By the end of this
                          process you will have finalized opt in page, tripwire
                          page and a purchase confirmation page copy read to
                          build your funnel with. If you get stuck with the
                          questions or are missing depth, come to our Messaging
                          Strategy Call and workshop it!
                        </p>
                      </CardContent>
                    </CollapsibleContent>
                  </Card>
                ...
                >
                  ...
                  >
                    ...
                    <CollapsibleContent>
                      <CardContent className="pt-0">
                        <p className="text-slate-600">
                          Use the copy generator to create draft email sequence
                          copy for your funnel. Create the draft and then edit
                          it so you have fully complete email copy ready to load
                          into your CRM. If you get stuck, leverage our
                    ...
                    <CollapsibleContent>
                      <CardContent className="pt-0">
                        <p className="text-slate-600 mb-4">
                          Take your finalized copy and create your funnel using
                          our easy templates
                        </p>
                ...
                >
                  ...
                  >
                    ...
                    <CollapsibleContent>
                      <CardContent className="pt-0">
                        <p className="text-slate-600">
                          Load your emails into your CRM or GHL platform and
                          connect your funnel so everything is working. Check
                          all the links in the emails, buttons on the page and
                          make sure it's all set up. If you get stuck come to
                          our tech support call for support!
                        </p>
                      </CardContent>
                    </CollapsibleContent>
                  </Card>
                ...
                >
                  ...
                  >
                    ...
                    <CollapsibleContent>
                      <CardContent className="pt-0">
                        <p className="text-slate-600">
                          Test your funnel as a new lead and make sure
                          everything is working correctly. Test every button,
                          make sure you got the email correctly and place a test
                          purchase of the product. Make sure to check your pages
                          on both desktop and mobile. Your funnel should be
                          fully functioning and ready for ads at this point! If
                          something with your tech is not working come to our
                          tech support call.
                        </p>
                      </CardContent>
                    </CollapsibleContent>
