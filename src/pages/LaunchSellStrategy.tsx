...
const CharacterCounter = ({ current, max }: { current: number; max: number }) => {
  ...
  return (
    <div className={`text-xs mt-1 ${isNearLimit ? 'text-red-600 font-medium' : 'text-slate-500'}`}>
...
export default function LaunchSellStrategy() {
  ...
  const { data: messagingStrategy, refetch: refetchMessagingStrategy } = useActiveMessagingStrategy(userId);
  
  // Load core offer outline (most recent one, regardless of active status)
  const { data: coreOfferOutline, isLoading: isLoadingOfferOutline, refetch: refetchCoreOfferOutline } = useQuery<any>({
  ...
  useEffect(() => {
    console.log('[CHECKBOX] useEffect triggered. savedCheckboxes:', savedCheckboxes);
    if (savedCheckboxes?.checkboxStates) {
      console.log('[CHECKBOX] Loading saved checkbox states:', savedCheckboxes.checkboxStates);
      setCheckboxStates(savedCheckboxes.checkboxStates);
      console.log('[CHECKBOX] State updated to:', savedCheckboxes.checkboxStates);
    } else {
      console.log('[CHECKBOX] No saved checkbox states found. savedCheckboxes:', savedCheckboxes);
    }
  }, [savedCheckboxes]);
  ...
  const [generatedSalesPageCopy, setGeneratedSalesPageCopy] = useState<string | null>(null);
  const [isEditingSalesPage, setIsEditingSalesPage] = useState(false);
  const [editedSalesPageCopy, setEditedSalesPageCopy] = useState<string | null>(null);
  ...
  const [editedEmailContent, setEditedEmailContent] = useState({ subject: "", body: "" });
  ...
  useEffect(() => {
    ...
    if (savedData && !initialDataLoadedRef.current) {
      console.log('[LAUNCH FUNNEL] Setting funnel data from saved data (initial load)');
      ...
      if (savedData.generatedOptInPage && savedData.generatedThankYouPage) {
        ...
        setGeneratedCopy({
          ...
          thankYouPage: stripMarkdownCodeFences(savedData.generatedThankYouPage)
        });
      }
      ...
      if (savedData.generatedSalesPageCopy) {
        ...
        let loadedCopy = stripMarkdownCodeFences(savedData.generatedSalesPageCopy);
  ...
  const saveMutation = useMutation({
    mutationFn: async (data: typeof funnelData) => {
      ...
      const response = await apiRequest("POST", "/api/launch-registration-funnel-data", data);
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["/api/launch-registration-funnel-data"] });
    },
    onError: (error) => {
  ...
  const saveToIgniteDocs = async (optInPage: string, thankYouPage: string): Promise<boolean> => {
    if (!userId) {
      console.error('[LAUNCH FUNNEL] Cannot save to IGNITE Docs: user not logged in');
      toast({
        title: "Save Warning",
    ...
    try {
      ...
      toast({
        ...
        description: "Your copy was generated but could not be saved to IGNITE Docs. Please try again.",
        variant: "destructive"
      });
      return false;
    }
  ...
  const generateCopyMutation = useMutation({
    mutationFn: async () => {
      ...
      const response = await apiRequest("POST", "/api/launch-registration-funnel/generate-copy", {
        messagingStrategy: latestMessagingStrategy.content,
    ...
    onSuccess: async (data) => {
      ...
      try {
        ...
        console.error('[LAUNCH FUNNEL] Error saving generated copy to database:', error);
        toast({
          title: "Database Save Failed",
          description: "Failed to save your copy to the database. Please try again.",
          variant: "destructive"
        });
      }
      ...
      if (dbSaveSuccess) {
        igniteSaveSuccess = await saveToIgniteDocs(cleanedData.optInPage, cleanedData.thankYouPage);
      }
      ...
      if (dbSaveSuccess && igniteSaveSuccess) {
        toast({
          ...
          description: "Your opt-in and thank you page copy has been saved to IGNITE Docs.",
        });
      } else if (dbSaveSuccess && !igniteSaveSuccess) {
        toast({
          title: "Partially Saved",
          description: "Your copy was generated and saved, but could not be saved to IGNITE Docs.",
          variant: "destructive"
        });
      }
    },
  ...
  const saveSalesPageToIgniteDocs = async (salesPageCopy: string): Promise<boolean> => {
    console.log('[SALES PAGE] saveSalesPageToIgniteDocs called with userId:', userId);
    
    if (!userId) {
      console.error('[SALES PAGE] Cannot save to IGNITE Docs: user not logged in');
      toast({
        title: "Save Warning",
        description: "Could not save to IGNITE Docs: Please log in again.",
        variant: "destructive"
    ...
    try {
      ...
      queryClient.invalidateQueries({ queryKey: ['/api/ignite-docs', 'user', userId] });
      console.log('[SALES PAGE] Copy automatically saved to IGNITE Docs successfully');
      return true;
    } catch (error) {
  ...
  const generateSalesPageMutation = useMutation({
    mutationFn: async () => {
      ...
      // Refetch messaging strategy to ensure we have the latest data
      const { data: latestMessagingStrategy } = await refetchMessagingStrategy();
      console.log('[SALES PAGE] Messaging strategy check:', latestMessagingStrategy ? 'Found' : 'Missing');
      ...
      console.log('[SALES PAGE] Core offer outline check:', latestCoreOfferOutline ? 'Found' : 'Missing');
      console.log('[SALES PAGE] Core offer outline data:', latestCoreOfferOutline);
      ...
      console.log('[SALES PAGE] All validation checks passed. Generating copy...');
      const response = await apiRequest("POST", "/api/launch-sales-page/generate-copy", {
    ...
    onSuccess: async (data) => {
      ...
      try {
        ...
        console.error('[SALES PAGE] Error saving generated copy to database:', error);
        toast({
          title: "Database Save Failed",
          description: "Failed to save your sales page copy to the database. Please try again.",
          variant: "destructive"
        });
      }
      ...
      if (dbSaveSuccess && igniteSaveSuccess) {
        ...
        toast({
          title: "Partially Saved",
          description: "Your sales page copy was generated and saved, but could not be saved to IGNITE Docs.",
          variant: "destructive"
        });
      }
    ...
    onError: (error: any) => {
      toast({
        title: "Generation Failed",
        description: error.message || "Failed to generate sales page copy. Please try again.",
        variant: "destructive",
      });
    }
  });
  ...
  const generateEmailsMutation = useMutation({
    mutationFn: async () => {
      ...
      try {
        const response = await fetch("/api/launch-emails/generate-sequence", {
          method: "POST",
        ...
        if (!response.ok) {
          const errorData = await response.json().catch(() => ({ message: 'Failed to generate emails' }));
    ...
    onSuccess: async (data) => {
      ...
      await queryClient.invalidateQueries({ queryKey: [`/api/launch-emails/${userId}`] });
      ...
      try {
        ...
        const emailContent = data.emails.map((email: any) => {
        ...
        await queryClient.invalidateQueries({ queryKey: ['/api/ignite-docs', 'user', userId] });
    ...
    onError: (error: any) => {
      ...
      toast({
        title: "Generation Failed",
        description: error.message || "Failed to generate email sequence. Please try again.",
        variant: "destructive",
      });
    }
  });
  ...
  const parseHTMLToRuns = (html: string, stripEmojis: boolean = false): Array<{ text: string; bold: boolean }> => {
    const temp = document.createElement('div');
    temp.innerHTML = html;
    const runs: Array<{ text: string; bold: boolean }> = [];
    
    // Block-level elements that should create line breaks
    const blockElements = ['P', 'DIV', 'H1', 'H2', 'H3', 'H4', 'H5', 'H6', 'UL', 'OL', 'LI'];
  ...
  const htmlToDOCXParagraphs = (html: string): Paragraph[] => {
    ...
    lines.forEach((line) => {
      const textRuns = line.map(segment => new TextRun({
        text: segment.text,
        bold: segment.bold,
  ...
  const exportEmailsAsPDF = () => {
    ...
    ['registration_invite', 'confirmation', 'nurture', 'reminder', 'sales'].forEach(emailType => {
      const emailsOfType = generatedEmails.filter(e => e.emailType === emailType);
    ...
    ['registration_invite', 'confirmation', 'nurture', 'reminder', 'sales'].forEach(emailType => {
      ...
      emailsOfType.forEach((email: any) => {
        ...
        children.push(
          new Paragraph({
            children: [
              new TextRun({ text: 'Email Body:', bold: true })
            ],
    ...
    <div>
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-bold text-slate-900 mb-2">Launch & Sell Strategy</h2>
            <p className="text-lg text-slate-600">Build your launch strategy to maximize sales and revenue!</p>
          </div>
            ...
            <Card>
              ...
              <CardContent>
                <div className="prose max-w-none text-slate-700 leading-relaxed space-y-4">
                  <p>
                    It's time to build your Live Launch Funnel — the engine that takes your new leads and turns them into buyers of your core offer!
                  </p>
                  <p>
                    You'll create the live launch opt-in, build out the full launch funnel, design your launch delivery, and craft a sales page that converts for your core offer.
                  </p>
                  <p>
                    By the end, you'll have everything in place and be fully ready to start ads!
                  </p>
                  <p>
                    You've been working so hard and this is such a big milestone! A live launch can be a lot of work but trust our process and we're here to support every step of the way.
                  </p>
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-center">
                  <p className="text-2xl font-bold text-orange-600 mb-2">14–30 Days</p>
                  <p className="text-sm text-slate-600">
                    Depending on how much copy you already have and your pace for building funnel pages, allow at least two weeks to get your launch assets created, set up, and tested.
                  </p>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent>
                <div className="space-y-3">
                  <div className="flex items-start gap-3">
                    ...
                    <p className="text-slate-700">Watch the training videos for creating & building a live launch funnel.</p>
                  </div>
                  <div className="flex items-start gap-3">
                    ...
                    <p className="text-slate-700">Generate copy for your live launch opt-in page using the Copy Generator.</p>
                  </div>
                  <div className="flex items-start gap-3">
                    ...
                    <p className="text-slate-700">Build your live launch funnel pages (opt-in, confirmation, delivery, etc.).</p>
                  </div>
                  <div className="flex items-start gap-3">
                    <div className="w-6 h-6 rounded-full bg-green-500 text-white text-sm font-medium flex items-center justify-center flex-shrink-0 mt-0.5">
                      4
                    </div>
                    <p className="text-slate-700">Create your live launch delivery (training, workshop, webinar, or challenge).</p>
                  </div>
                  <div className="flex items-start gap-3">
                    <div className="w-6 h-6 rounded-full bg-blue-500 text-white text-sm font-medium flex items-center justify-center flex-shrink-0 mt-0.5">
                      5
                    </div>
                    <p className="text-slate-700">Generate sales page copy using the Copy Generator.</p>
                  </div>
                  <div className="flex items-start gap-3">
                    <div className="w-6 h-6 rounded-full bg-orange-500 text-white text-sm font-medium flex items-center justify-center flex-shrink-0 mt-0.5">
                      6
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <p className="text-slate-700">Write your live launch emails.</p>
                  </div>
                  <div className="flex items-start gap-3">
                    ...
                    <p className="text-slate-700">Connect all tech (emails, tags, automations, payment links).</p>
                  </div>
                  <div className="flex items-start gap-3">
                    ...
                    <p className="text-slate-700">Test the full funnel + delivery experience to confirm everything works.</p>
                  </div>
                  <div className="flex items-start gap-3">
                    ...
                    <p className="text-slate-700">Finalize assets and be ready to start ads for your live launch.</p>
                  </div>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent>
                <div className="space-y-3">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <strong className="text-slate-900">Deliver transformation, not just information</strong>
                      <span className="text-slate-600"> → Whether it's a webinar, challenge, or workshop, design your live launch delivery so participants feel connected to you.</span>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    <span className="text-yellow-500 mt-1">→</span>
                    <div>
                      <strong className="text-slate-900">Messaging Is Key</strong>
                      <span className="text-slate-600"> → Ensure your opt-in, delivery, and sales page all speak directly to the same pain point and desired outcome your audience cares about most.</span>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    <span className="text-yellow-500 mt-1">→</span>
                    <div>
                      <strong className="text-slate-900">Keep it simple and clear</strong>
                      <span className="text-slate-600"> → A sales page doesn't need to be overwhelming. Focus on the big promise, proof, and a clear call-to-action.</span>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    <span className="text-yellow-500 mt-1">→</span>
                    <div>
                      <strong className="text-slate-900">Practice the journey</strong>
                      <span className="text-slate-600"> → Walk through the funnel as if you're a new lead to ensure everything flows seamlessly from sign-up to sales page.</span>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    <span className="text-yellow-500 mt-1">→</span>
                    <div>
                      <strong className="text-slate-900">Leverage Our Team!</strong>
                      <span className="text-slate-600"> → There is no denying that a live launch can be overwhelming, especially if it's your first one. Use our team, show up messy and don't allow the overwhelm to cause paralysis. You will learn so much just walking through this experience!</span>
                    </div>
                  </div>
                </div>
              </CardContent>
          ...
          <Card>
            ...
            <CardContent>
              <p className="text-lg text-slate-700 mb-4">
                Watch these training videos for a complete breakdown of successful live launch strategies
              </p>
              
              {/* Nested Video Training Tabs */}
              <Tabs defaultValue="live-launch-strategy" className="space-y-4">
                <TabsList className="grid w-full grid-cols-4 gap-1">
                  <TabsTrigger value="live-launch-strategy" className="text-xs md:text-sm px-2 md:px-4">Live Launch Strategy</TabsTrigger>
                  <TabsTrigger value="webinar" className="text-xs md:text-sm px-2 md:px-4">Live Launch: Webinar</TabsTrigger>
                  <TabsTrigger value="challenge" className="text-xs md:text-sm px-2 md:px-4">Live Launch: Challenge</TabsTrigger>
                  <TabsTrigger value="tech-setup" className="text-xs md:text-sm px-2 md:px-4">Live Launch Funnel Tech Set Up</TabsTrigger>
                </TabsList>

                <TabsContent value="live-launch-strategy" className="space-y-6">
                  <div className="space-y-4">
                    <h3 className="text-xl font-semibold text-slate-900">Live Launch Strategy</h3>
                    <p className="text-slate-700">
                      Master the fundamentals of creating a successful live launch funnel that converts leads into customers.
                    </p>
                ...
                <TabsContent value="webinar" className="space-y-6">
                  <div className="space-y-4">
                    <h3 className="text-xl font-semibold text-slate-900">Live Launch: Webinar</h3>
                ...
                <TabsContent value="challenge" className="space-y-6">
                  <div className="space-y-4">
                    <h3 className="text-xl font-semibold text-slate-900">Live Launch: Challenge</h3>
                ...
                <TabsContent value="tech-setup" className="space-y-6">
                  <div className="space-y-6">
                    <h3 className="text-xl font-semibold text-slate-900">Live Launch Challenge Live Launch Funnel Tech Setup</h3>
          ...
          <Card>
            ...
            <CardContent>
              <p className="text-slate-700 mb-4">
                Use our tool to generate all of your live launch copy from the funnel to the sales page to the emails!
              </p>
              
              {/* Nested Copy Generator Tabs */}
              <Tabs defaultValue="registration-funnel" className="space-y-4">
                <TabsList className="grid w-full grid-cols-3 gap-1">
                  <TabsTrigger value="registration-funnel" className="text-xs md:text-sm px-2 md:px-4">Launch Registration Funnel</TabsTrigger>
                  <TabsTrigger value="sales-page" className="text-xs md:text-sm px-2 md:px-4">Sales Page</TabsTrigger>
                  <TabsTrigger value="launch-emails" className="text-xs md:text-sm px-2 md:px-4">Launch Emails</TabsTrigger>
                </TabsList>
                  ...
                  <div className="space-y-8">
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="flex items-center justify-between flex-wrap gap-4">
                        <h2 className="text-2xl font-bold text-slate-900">Launch Registration Funnel Generator</h2>
                            ...
                            {isEditingCopy ? (
                              ...
                              <>
                                ...
                                <DropdownMenu>
                                  ...
                                  <DropdownMenuContent>
                                    <DropdownMenuItem onClick={exportAsPDF} data-testid="button-export-pdf" className="hover:bg-gray-100 focus:bg-gray-100">
                                      <FileText className="w-4 h-4 mr-2" />
                                      Download as PDF
                                    </DropdownMenuItem>
                                    <DropdownMenuItem onClick={exportAsDOCX} data-testid="button-export-docx" className="hover:bg-gray-100 focus:bg-gray-100">
                                      <FileText className="w-4 h-4 mr-2" />
                                      Download as DOCX
                                    </DropdownMenuItem>
                                  </DropdownMenuContent>
                      ...
                      {generatedCopy && (
                        <Card ref={generatedCopyRef} className="border-2 border-orange-200">
                            ...
                            <div>
                              {isEditingCopy && editedCopy ? (
                                <Textarea
                                  ...
                                  onChange={(e) => setEditedCopy(prev => prev ? { ...prev, optInPage: e.target.value } : null)}
                                ...
                                <div 
                                  ...
                                  dangerouslySetInnerHTML={{ __html: generatedCopy.optInPage }}
                            ...
                            <div>
                              {isEditingCopy && editedCopy ? (
                                <Textarea
                                  ...
                                  onChange={(e) => setEditedCopy(prev => prev ? { ...prev, thankYouPage: e.target.value } : null)}
                                  className="min-h-[300px] bg-white p-6 rounded-lg border font-mono text-sm"
                                ...
                                <div 
                                  className="bg-white p-6 rounded-lg border whitespace-pre-line"
                                  dangerouslySetInnerHTML={{ __html: generatedCopy.thankYouPage }}
                      ...
                      <Card>
                        ...
                        <CardContent className="space-y-4">
                          <div className="space-y-3">
                            <Label htmlFor="launchDateTime">1. When is the date and time of your live launch experience? Please include the time zone.</Label>
                            <Input 
                              id="launchDateTime" 
                          ...
                          <div className="space-y-3">
                            <Label htmlFor="experienceType">2. What type of experience is this? Webinar, series, challenge, etc.</Label>
                            <Input 
                              id="experienceType" 
                              placeholder="Enter the type of experience..." 
                              value={funnelData.experienceType}
                              onChange={(e) => updateField('experienceType', e.target.value)}
                              data-testid="input-experience-type"
                            />
                          </div>
                          
                          <div className="space-y-3">
                            <Label htmlFor="transformationResult">3. In one sentence, what result or transformation will someone get from this experience? The main outcome and use your ICA's language!</Label>
                            <Textarea 
                              ...
                              onChange={(e) => updateField('transformationResult', e.target.value)}
                              data-testid="textarea-transformation-result"
                            />
                            <CharacterCounter current={funnelData.transformationResult.length} max={CHARACTER_LIMITS.transformationResult} />
                          </div>
                          
                          <div className="space-y-3">
                            <Label htmlFor="topThreeOutcomes">4. What are the top 3 things they will be able to know/be/do after participating in this live launch?</Label>
                            <Textarea 
                              id="topThreeOutcomes" 
                              ...
                              onChange={(e) => updateField('topThreeOutcomes', e.target.value)}
                              data-testid="textarea-top-three-outcomes"
                            />
                            <CharacterCounter current={funnelData.topThreeOutcomes.length} max={CHARACTER_LIMITS.topThreeOutcomes} />
                          </div>
                          
                          <div className="space-y-3">
                            <Label htmlFor="specificProblem">5. What specific problem does it solve for your ideal customer right now?</Label>
                            <Textarea 
                              id="specificProblem" 
                            ...
                            />
                            <CharacterCounter current={funnelData.specificProblem.length} max={CHARACTER_LIMITS.specificProblem} />
                          </div>
                          
                          <div className="space-y-3">
                            <Label htmlFor="urgentProblem">6. Why is this problem urgent or important to solve?</Label>
                            <Textarea 
                              id="urgentProblem" 
                              ...
                              onChange={(e) => updateField('urgentProblem', e.target.value)}
                              data-testid="textarea-urgent-problem"
                            />
                            <CharacterCounter current={funnelData.urgentProblem.length} max={CHARACTER_LIMITS.urgentProblem} />
                          </div>
                          
                          <div className="space-y-3">
                            <Label htmlFor="uniqueExperience">7. What's unique about this experience? Think about the outcome and how you help them achieve that outcome, why is that better than other options out there?</Label>
                            <Textarea 
                              id="uniqueExperience" 
                            ...
                            />
                            <CharacterCounter current={funnelData.uniqueExperience.length} max={CHARACTER_LIMITS.uniqueExperience} />
                          </div>
                          
                          <div className="space-y-3">
                            <Label htmlFor="showUpBonus">8. Will you have a show up bonus for people who show up and participate live? If so, what is the OUTCOME of this show up bonus? Why do I want it?</Label>
                            <Textarea 
                              id="showUpBonus" 
                              ...
                              onChange={(e) => updateField('showUpBonus', e.target.value)}
                              data-testid="textarea-show-up-bonus"
                            />
                            <CharacterCounter current={funnelData.showUpBonus.length} max={CHARACTER_LIMITS.showUpBonus} />
                          </div>
                          
                          <div className="space-y-3">
                            <Label htmlFor="thankYouAction">9. Where would you like to send people on the thank you page of your funnel? (Ex: DM you on Instagram, FB group or just to add the event to their calendar). Provide details on why they should take this action</Label>
                            <Textarea 
                              id="thankYouAction" 
                              ...
                              onChange={(e) => updateField('thankYouAction', e.target.value)}
                              data-testid="textarea-thank-you-action"
                            />
                            <CharacterCounter current={funnelData.thankYouAction.length} max={CHARACTER_LIMITS.thankYouAction} />
                          </div>
                        </CardContent>
                      </Card>
                      ...
                      <Card>
                        ...
                        <CardContent className="space-y-4">
                          <div className="space-y-3">
                            ...
                            />
                            <CharacterCounter current={funnelData.painPoints.length} max={CHARACTER_LIMITS.painPoints} />
                          </div>
                          
                          <div className="space-y-3">
                            <Label htmlFor="quickWin">2. What's the quick win they'll walk away with after participating?</Label>
                            <Textarea 
                              id="quickWin" 
                            ...
                            />
                            <CharacterCounter current={funnelData.quickWin.length} max={CHARACTER_LIMITS.quickWin} />
                          </div>
                          ...
                          <div className="space-y-3">
                            ...
                            />
                            <CharacterCounter current={funnelData.objections.length} max={CHARACTER_LIMITS.objections} />
                          </div>
                          
                          <div className="space-y-3">
                            <Label htmlFor="socialProofResults">5. Do you have any results or social proof that you'd use to showcase how great this experience will be?</Label>
                            <Textarea 
                              ...
                              onChange={(e) => updateField('socialProofResults', e.target.value)}
                              data-testid="textarea-social-proof-results"
                            />
                            <CharacterCounter current={funnelData.socialProofResults.length} max={CHARACTER_LIMITS.socialProofResults} />
                          </div>
                        </CardContent>
                      </Card>
                      ...
                      <div className="flex justify-center pt-6">
                        <Button 
                          ...
                          onClick={() => {
                            if (hasCharacterLimitViolations()) {
                              toast({
                                ...
                                description: "Please reduce the content in fields marked in red before generating.",
                                variant: "destructive"
                              });
                              return;
                            }
                          ...
                          disabled={generateCopyMutation.isPending || hasCharacterLimitViolations()}
                          data-testid="button-generate-funnel-copy"
                        >
                          <Zap className="w-5 h-5 mr-2" />
                          {generateCopyMutation.isPending ? "Generating..." : "Generate Registration Funnel Copy"}
                        </Button>
                      </div>
                    </div>
                  </div>
                ...
                <TabsContent value="sales-page" className="space-y-6">
                  <div className="space-y-8">
                                ...
                                {isEditingSalesPage ? (
                                  ...
                                  <>
                                    ...
                                    <DropdownMenu>
                                      ...
                                      <DropdownMenuContent>
                                        <DropdownMenuItem onClick={exportSalesPageAsPDF} data-testid="button-export-sales-pdf" className="hover:bg-gray-100 focus:bg-gray-100">
                                          <FileText className="w-4 h-4 mr-2" />
                                          Download as PDF
                                        </DropdownMenuItem>
                                        <DropdownMenuItem onClick={exportSalesPageAsDOCX} data-testid="button-export-sales-docx" className="hover:bg-gray-100 focus:bg-gray-100">
                                          <FileText className="w-4 h-4 mr-2" />
                                          Download as DOCX
                                        </DropdownMenuItem>
                                      </DropdownMenuContent>
                          ...
                          <CardContent className="pt-6">
                            {isEditingSalesPage && editedSalesPageCopy ? (
                              <Textarea
                                ...
                                onChange={(e) => setEditedSalesPageCopy(e.target.value)}
                              ...
                              <div 
                                className="bg-white p-6 rounded-lg border whitespace-pre-line"
                                dangerouslySetInnerHTML={{ __html: generatedSalesPageCopy }}
                    ...
                    {/* Sales Page Section */}
                    <div className="space-y-6">
                      <h2 className="text-2xl font-bold text-slate-900">Sales Page Generator</h2>
                      ...
                      <Card>
                        ...
                        <CardContent className="space-y-4">
                          <div className="space-y-3">
                            <Label htmlFor="salesPageAction">What do you want people to do after reading the page? (buy now, book a call, etc.)</Label>
                            <Textarea 
                              id="salesPageAction" 
                              ...
                              onChange={(e) => updateField('salesPageAction', e.target.value)}
                              data-testid="textarea-sales-page-action"
                            />
                            <CharacterCounter current={funnelData.salesPageAction.length} max={CHARACTER_LIMITS.salesPageAction} />
                          </div>
                          
                          <div className="space-y-3">
                            <Label htmlFor="salesPageUrgency">Do you have urgency or scarcity elements? (limited spots, enrollment deadline, bonuses expiring, etc.)</Label>
                            <Textarea 
                              id="salesPageUrgency" 
                              ...
                              onChange={(e) => updateField('salesPageUrgency', e.target.value)}
                              data-testid="textarea-sales-page-urgency"
                            />
                            <CharacterCounter current={funnelData.salesPageUrgency.length} max={CHARACTER_LIMITS.salesPageUrgency} />
                          </div>
                        </CardContent>
                      </Card>
                      ...
                      <div className="flex justify-center pt-6">
                        <Button 
                          ...
                          disabled={generateSalesPageMutation.isPending || !funnelData.salesPageAction || hasCharacterLimitViolations()}
                ...
                <TabsContent value="launch-emails" className="space-y-6">
                  <div className="space-y-8">
                        ...
                        <Card className="border-2 border-blue-200">
                          <CardHeader className="bg-blue-50">
                            <CardTitle className="flex items-center justify-between text-blue-900">
                              <span className="flex items-center gap-2">
                                <CheckCircle className="w-5 h-5 text-green-600" />
                                Your Launch Email Sequence ({generatedEmails.length} Emails)
                              </span>
                              <DropdownMenu>
                                <DropdownMenuTrigger asChild>
                                  <Button 
                          ...
                          <CardContent className="pt-6 space-y-6">
                            {['registration_invite', 'confirmation', 'nurture', 'reminder', 'sales'].map(emailType => {
                              const emailsOfType = generatedEmails.filter(e => e.emailType === emailType);
                              ...
                              return (
                                <div key={emailType} className="space-y-4">
                                  ...
                                  {emailsOfType.map((email) => (
                                    <Card key={email.id} className="border border-slate-200">
                                      <CardHeader className="bg-slate-50 pb-3">
                                        <CardTitle className="text-base flex items-center justify-between">
                                          <span className="text-slate-700">Email {email.emailNumber}</span>
                                          {editingEmailId === email.id ? (
                                            <div className="flex gap-2">
                                              <Button 
                                                ...
                                                onClick={async () => {
                                                  try {
                                                    await apiRequest("PUT", `/api/launch-emails/${email.id}`, editedEmailContent);
                                                    const updatedEmails = generatedEmails.map(e => 
                                                      e.id === email.id ? { ...e, ...editedEmailContent } : e
                                                    );
                                              ...
                                              <Button 
                                                ...
                                                onClick={() => {
                                                  setEditingEmailId(null);
                                                  setEditedEmailContent({ subject: "", body: "" });
                                                }}
                                                className="bg-embodied-coral hover:bg-embodied-orange text-white"
                                              >
                                                Cancel
                                      ...
                                      <CardContent className="pt-4 space-y-4">
                                        {editingEmailId === email.id ? (
                                          ...
                                          <>
                                            <div>
                                              <span className="text-sm font-semibold text-slate-700">Subject Line:</span>
                                              <p className="text-slate-900 mt-1 font-medium">{email.subject}</p>
                                            </div>
                                            <div>
                    ...
                    <div className="space-y-6">
                      <h2 className="text-2xl font-bold text-slate-900">Launch Email Templates</h2>
                      
                      {/* Email Overview Note */}
                      <Collapsible defaultOpen={true}>
                        <CollapsibleTrigger asChild>
                          <Button variant="outline" className="w-full justify-between border-blue-200 bg-blue-50 hover:bg-blue-100">
                            <div className="flex items-center gap-2">
                              <Info className="w-4 h-4 text-blue-600" />
                              <span className="text-blue-900 font-medium">Live Launch Email Sequence Overview</span>
                            </div>
                            ...
                            <CardContent className="pt-6">
                              <div className="space-y-4 text-slate-700">
                                <p className="font-medium text-blue-900">
                                  We'll be writing 5 different types of emails for your live launch email sequence. Please remember the tool is meant to get this copy 70% of the way there and you finish it off!
                                </p>
                                
                                <div className="space-y-4">
                                  <p className="font-semibold text-slate-800">Here are the emails you'll need for your live launch:</p>
                                  
                                  <div className="space-y-3">
                                    <div>
                                      <h4 className="font-semibold text-slate-800 mb-1">📩 Registration Invite Emails (5)</h4>
                                      <p className="text-sm mb-1"><strong>Purpose:</strong> Drives your leads to sign up for the live launch event.</p>
                                      <ul className="text-sm space-y-1 ml-4">
                                        <li>• Each email highlights a different hook, pain point, or desired outcome.</li>
                                        <li>• Overcomes objections and creates urgency to register.</li>
                                        <li>• Builds anticipation around the transformation the event promises.</li>
                                      </ul>
                                    </div>
                                    
                                    <div>
                                      <h4 className="font-semibold text-slate-800 mb-1">📩 Confirmation Email (1)</h4>
                                      <p className="text-sm mb-1"><strong>Purpose:</strong> Welcome new registrants and lock in attendance.</p>
                                      <ul className="text-sm space-y-1 ml-4">
                                        <li>• Confirms date, time, and access details.</li>
                                        <li>• Reinforces the value of showing up live.</li>
                                        <li>• Encourages next steps (join group, add to calendar, send DM, etc.).</li>
                                      </ul>
                                    </div>
                                    
                                    <div>
                                      <h4 className="font-semibold text-slate-800 mb-1">📩 Nurture Emails (3)</h4>
                                      <p className="text-sm mb-1"><strong>Purpose:</strong> Build excitement and trust before the event.</p>
                                      <ul className="text-sm space-y-1 ml-4">
                                        <li>• Share your story, authority, or client success stories.</li>
                                        <li>• Break down limiting beliefs or myths holding your audience back.</li>
                                        <li>• Warm up registrants so they show up ready to engage.</li>
                                      </ul>
                                    </div>
                                    
                                    <div>
                                      <h4 className="font-semibold text-slate-800 mb-1">📩 Reminder Emails (3)</h4>
                                      <p className="text-sm mb-1"><strong>Purpose:</strong> Maximize live attendance.</p>
                                      <ul className="text-sm space-y-1 ml-4">
                                        <li>• Sent at key times (24 hours before, 1 hour before & live now).</li>
                                        <li>• Reinforce what they'll gain by showing up live.</li>
                                      </ul>
                                    </div>
                                    
                                    <div>
                                      <h4 className="font-semibold text-slate-800 mb-1">📩 Sales Emails (5)</h4>
                                      <p className="text-sm mb-1"><strong>Purpose:</strong> Convert attendees and registrants into buyers of your core offer.</p>
                                      <ul className="text-sm space-y-1 ml-4">
                                        <li>• Highlight the big promise and transformation of your offer.</li>
                                        <li>• Show what's included and why it's valuable.</li>
                                        <li>• Overcome objections with proof, testimonials, and guarantees.</li>
                                        <li>• Drive urgency with cart close dates, limited spots, or bonuses.</li>
                                      </ul>
                                    </div>
                                  </div>
                                </div>
                      ...
                      <Card>
                        ...
                        <CardContent className="space-y-4">
                          <div className="space-y-3">
                            <Label htmlFor="inviteHooks">List out some main hooks and promises we can use in your emails to get leads to sign up. Why should somebody sign up for this experience right now?</Label>
                            <Textarea 
                              id="inviteHooks" 
                              ...
                              onChange={(e) => updateEmailInput('inviteHooks', e.target.value)}
                              data-testid="textarea-invite-hooks"
                            />
                            <CharacterCounter current={emailInputs.inviteHooks.length} max={CHARACTER_LIMITS.inviteHooks} />
                          </div>
                          
                          <div className="space-y-3">
                            <Label htmlFor="inviteFOMO">What are they missing out on if they miss this experience?</Label>
                            <Textarea 
                              id="inviteFOMO" 
                              ...
                              onChange={(e) => updateEmailInput('inviteFOMO', e.target.value)}
                              data-testid="textarea-invite-fomo"
                            />
                            <CharacterCounter current={emailInputs.inviteFOMO.length} max={CHARACTER_LIMITS.inviteFOMO} />
                          </div>
                        </CardContent>
                      </Card>
                      ...
                      <Card>
                        ...
                        <CardContent className="space-y-4">
                          <div className="space-y-3">
                            <Label htmlFor="confirmationDetails">What key details do registrants need immediately? (time, date, link, how to prepare)</Label>
                            <Textarea 
                              ...
                              onChange={(e) => updateEmailInput('confirmationDetails', e.target.value)}
                              data-testid="textarea-confirmation-details"
                            />
                            <CharacterCounter current={emailInputs.confirmationDetails.length} max={CHARACTER_LIMITS.confirmationDetails} />
                          </div>
                          
                          <div className="space-y-3">
                            <Label htmlFor="preEventActions">Is there something they should do before the event? (join a group, add event to calendar, DM you, etc.)</Label>
                            <Textarea 
                              id="preEventActions" 
                              ...
                              onChange={(e) => updateEmailInput('preEventActions', e.target.value)}
                              data-testid="textarea-pre-event-actions"
                            />
                            <CharacterCounter current={emailInputs.preEventActions.length} max={CHARACTER_LIMITS.preEventActions} />
                          </div>
                        </CardContent>
                      </Card>
                      ...
                      <Card>
                        ...
                        <CardContent className="space-y-4">
                          <div className="space-y-3">
                            <Label htmlFor="nurtureContent">What stories, tips, or insights can you share that build excitement before the event?</Label>
                            <Textarea 
                              id="nurtureContent" 
                              ...
                              onChange={(e) => updateEmailInput('nurtureContent', e.target.value)}
                              data-testid="textarea-nurture-content"
                            />
                            <CharacterCounter current={emailInputs.nurtureContent.length} max={CHARACTER_LIMITS.nurtureContent} />
                          </div>
                          
                          <div className="space-y-3">
                            <Label htmlFor="liveAttendanceValue">WHY should somebody show up live for this event? What would you tell a friend on why they can't afford to miss this?</Label>
                            <Textarea 
                              ...
                              onChange={(e) => updateEmailInput('liveAttendanceValue', e.target.value)}
                              data-testid="textarea-live-attendance-value"
                            />
                            <CharacterCounter current={emailInputs.liveAttendanceValue.length} max={CHARACTER_LIMITS.liveAttendanceValue} />
                          </div>
                          
                          <div className="space-y-3">
                            <Label htmlFor="mythsBeliefs">What common myths or limiting beliefs do you want to start breaking down before they attend?</Label>
                            <Textarea 
                              id="mythsBeliefs" 
                              ...
                              onChange={(e) => updateEmailInput('mythsBeliefs', e.target.value)}
                              data-testid="textarea-myths-beliefs"
                            />
                            <CharacterCounter current={emailInputs.mythsBeliefs.length} max={CHARACTER_LIMITS.mythsBeliefs} />
                          </div>
                        </CardContent>
                      </Card>
                      ...
                      <Card>
                        ...
                        <CardContent className="space-y-4">
                          <div className="space-y-3">
                            <Label htmlFor="salesStories">Are there any stories you feel would fit well in your sales emails? It's okay if they are not official testimonials.</Label>
                            <Textarea 
                              id="salesStories" 
                              ...
                              onChange={(e) => updateEmailInput('salesStories', e.target.value)}
                              data-testid="textarea-sales-stories"
                            />
                            <CharacterCounter current={emailInputs.salesStories.length} max={CHARACTER_LIMITS.salesStories} />
                          </div>
                          
                          <div className="space-y-3">
                            <Label htmlFor="finalPush">What will get people on the fence over the edge? What would you say to a friend or potential client who's been reading all the emails and hasn't said yes yet?</Label>
                            <Textarea 
                              id="finalPush" 
                              ...
                              onChange={(e) => updateEmailInput('finalPush', e.target.value)}
                              data-testid="textarea-final-push"
                            />
                            <CharacterCounter current={emailInputs.finalPush.length} max={CHARACTER_LIMITS.finalPush} />
                          </div>
                        </CardContent>
                      </Card>
                      ...
                      <div className="flex justify-center pt-6">
                        <Button 
                          ...
                          disabled={generateEmailsMutation.isPending || !emailInputs.inviteHooks || hasCharacterLimitViolations()}
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                ...
                <div className="space-y-4">
                  {/* Task 1 */}
                  <Collapsible open={expandedTasks.task1} onOpenChange={() => handleTaskExpand('task1')}>
                        ...
                        <CardContent className="py-4 cursor-pointer hover:bg-slate-50">
                          <div className="flex items-center space-x-4">
                            <Checkbox 
                              ...
                              onCheckedChange={(checked) => handleCheckboxChange('task-1', !!checked)}
                            ...
                            <h4 className="font-semibold text-slate-800 flex-1">Create Your Launch Registration Funnel Copy</h4>
                            <ChevronDown className={`w-5 h-5 text-slate-400 transition-transform duration-200 ${expandedTasks.task1 ? 'transform rotate-180' : ''}`} />
                          </div>
                        </CardContent>
                      </CollapsibleTrigger>
                      <CollapsibleContent>
                        <CardContent className="pt-0 pb-4">
                          <div className="ml-16 space-y-2">
                            <p className="text-slate-600 text-sm">
                              Answer the questions in the launch registration funnel copy generator and get your first draft of copy created.
                            </p>
                            <p className="text-slate-600 text-sm">
                              Take that copy and finalize it. It's meant to get you 70% of the way there and you finish the last 30%! By the end of this process you will have finalized opt in and thank you page for your live launch experience.
                            </p>
                            <p className="text-slate-600 text-sm">
                  ...
                  {/* Task 2 */}
                  <Collapsible open={expandedTasks.task3} onOpenChange={() => handleTaskExpand('task3')}>
                    <Card className="border border-slate-200">
                      <CollapsibleTrigger asChild>
                        <CardContent className="py-4 cursor-pointer hover:bg-slate-50">
                          <div className="flex items-center space-x-4">
                            <Checkbox 
                              ...
                              onCheckedChange={(checked) => handleCheckboxChange('task-3', !!checked)}
                              onClick={(e) => e.stopPropagation()} 
                            ...
                            <h4 className="font-semibold text-slate-800 flex-1">Create Your Sales Page Copy</h4>
                            <ChevronDown className={`w-5 h-5 text-slate-400 transition-transform duration-200 ${expandedTasks.task3 ? 'transform rotate-180' : ''}`} />
                          </div>
                        </CardContent>
                      </CollapsibleTrigger>
                      <CollapsibleContent>
                        <CardContent className="pt-0 pb-4">
                          <div className="ml-16">
                            <p className="text-slate-600 text-sm">
                              Use our copy generator to draft your sales page copy. We will pull from your messaging strategy and irresistible offer outline to draft your initial copy. Use this to finalize your sales page copy!
                            </p>
                          </div>
                        </CardContent>
                      </CollapsibleContent>
                  ...
                  {/* Task 3 */}
                  <Collapsible open={expandedTasks.task4} onOpenChange={() => handleTaskExpand('task4')}>
                    <Card className="border border-slate-200">
                      <CollapsibleTrigger asChild>
                        <CardContent className="py-4 cursor-pointer hover:bg-slate-50">
                          <div className="flex items-center space-x-4">
                            <Checkbox 
                              ...
                              onCheckedChange={(checked) => handleCheckboxChange('task-4', !!checked)}
                              onClick={(e) => e.stopPropagation()} 
                            ...
                            <h4 className="font-semibold text-slate-800 flex-1">Build Your Sales Funnel</h4>
                            <ChevronDown className={`w-5 h-5 text-slate-400 transition-transform duration-200 ${expandedTasks.task4 ? 'transform rotate-180' : ''}`} />
                          </div>
                        </CardContent>
                      </CollapsibleTrigger>
                      <CollapsibleContent>
                        <CardContent className="pt-0 pb-4">
                          <div className="ml-16 space-y-4">
                            <p className="text-slate-600 text-sm">
                              Take your finalized copy and create your funnel using our easy templates. This will include your live launch opt in page, confirmation page, sales page and purchase confirmation page once you're finished.
                            </p>
                  ...
                  {/* Task 4 */}
                  <Collapsible open={expandedTasks.task2} onOpenChange={() => handleTaskExpand('task2')}>
                    <Card className="border border-slate-200">
                      <CollapsibleTrigger asChild>
                        <CardContent className="py-4 cursor-pointer hover:bg-slate-50">
                          <div className="flex items-center space-x-4">
                            <Checkbox 
                              ...
                              onCheckedChange={(checked) => handleCheckboxChange('task-2', !!checked)}
                              onClick={(e) => e.stopPropagation()} 
                            ...
                            <h4 className="font-semibold text-slate-800 flex-1">Write Your Email Copy</h4>
                            <ChevronDown className={`w-5 h-5 text-slate-400 transition-transform duration-200 ${expandedTasks.task2 ? 'transform rotate-180' : ''}`} />
                          </div>
                        </CardContent>
                      </CollapsibleTrigger>
                      <CollapsibleContent>
                        <CardContent className="pt-0 pb-4">
                          <div className="ml-16 space-y-2">
                            <p className="text-slate-600 text-sm">
                              Use the copy generator to create draft email copy for your funnel. You will end up with invite copy, a confirmation email, nurture emails and sales emails.
                            </p>
                            <p className="text-slate-600 text-sm">
                              Create the draft and then edit it so you have fully complete email copy ready to load into your automation. If you get stuck, leverage our Messaging Support Call!
                            </p>
                          </div>
                        </CardContent>
                      </CollapsibleContent>
                  ...
                  {/* Task 5 */}
                  <Collapsible open={expandedTasks.task5} onOpenChange={() => handleTaskExpand('task5')}>
                    <Card className="border border-slate-200">
                      <CollapsibleTrigger asChild>
                        <CardContent className="py-4 cursor-pointer hover:bg-slate-50">
                          <div className="flex items-center space-x-4">
                            <Checkbox 
                              ...
                              onCheckedChange={(checked) => handleCheckboxChange('task-5', !!checked)}
                              onClick={(e) => e.stopPropagation()} 
                            ...
                            <h4 className="font-semibold text-slate-800 flex-1">Set Up Your Tech</h4>
                            <ChevronDown className={`w-5 h-5 text-slate-400 transition-transform duration-200 ${expandedTasks.task5 ? 'transform rotate-180' : ''}`} />
                          </div>
                        </CardContent>
                      </CollapsibleTrigger>
                      <CollapsibleContent>
                        <CardContent className="pt-0 pb-4">
                          <div className="ml-16">
                            <p className="text-slate-600 text-sm">
                              Now that you have all your email copy ready you can set up your automations and load the emails into your CRM. Reference our training video on how to set up your emails in GHL. Your funnel should be fully ready for ads to launch once you've finished this step!
                            </p>
                          </div>
                        </CardContent>
                      </CollapsibleContent>
                  ...
                  {/* Task 6 */}
                  <Collapsible open={expandedTasks.task6} onOpenChange={() => handleTaskExpand('task6')}>
                    <Card className="border border-slate-200">
                      <CollapsibleTrigger asChild>
                        <CardContent className="py-4 cursor-pointer hover:bg-slate-50">
                          <div className="flex items-center space-x-4">
                            <Checkbox 
                              ...
                              onCheckedChange={(checked) => handleCheckboxChange('task-6', !!checked)}
                              onClick={(e) => e.stopPropagation()} 
                            ...
                            <h4 className="font-semibold text-slate-800 flex-1">Test Your Funnel</h4>
                            <ChevronDown className={`w-5 h-5 text-slate-400 transition-transform duration-200 ${expandedTasks.task6 ? 'transform rotate-180' : ''}`} />
                          </div>
                        </CardContent>
                      </CollapsibleTrigger>
                      <CollapsibleContent>
                        <CardContent className="pt-0 pb-4">
                          <div className="ml-16">
                            <p className="text-slate-600 text-sm">
                              Test your complete funnel to ensure everything works properly before launching ads.
                            </p>
                            <p className="text-slate-600 text-sm mt-2">
                              Opt in to it as a lead and click every link in your emails. Make sure to also check your pages on mobile and desktop.
                            </p>
                          </div>
                        </CardContent>
                      </CollapsibleContent>
